...
import { WaistHistoryGoalCardComponent } from './waist-history-goal-card';
...
): { fixture: ComponentFixture<WaistHistoryGoalCardComponent>; component: WaistHistoryGoalCardComponent; root: HTMLElement } {
    TestBed.configureTestingModule({ imports: [WaistHistoryGoalCardComponent], providers: [provideTranslateTesting()] });
    const fixture = TestBed.createComponent(WaistHistoryGoalCardComponent);
    for (const [name, value] of Object.entries({
        currentWaist: current,
        currentWaistDate: '2026-08-13',
        desiredWaistCm: goal,
        startWaistCm: start,
...
describe('Waist goal progress', () => {
    ...
    it('renders no-goal state and emits actions from real buttons', () => {
        ...
        expect(root.textContent).toContain('WAIST_HISTORY.GOAL_NOT_SET');
        const buttons = Array.from(root.querySelectorAll('button'));
        buttons.find(b => b.textContent.includes('WAIST_HISTORY.SET_GOAL'))?.click();
        buttons.find(b => b.textContent.includes('WAIST_HISTORY.GOAL_HISTORY'))?.click();
...
describe('Goal estimates and completed goals', () => {
    it('renders imperial values while keeping canonical calculations', () => {
        ...
        expect(root.textContent).toContain('GENERAL.UNITS.IN');
    });
    ...
    it('handles a missing latest date and a valid weekly estimate', () => {
        ...
        fixture.componentRef.setInput('currentWaistDate', null);
        fixture.detectChanges();
        expect(component['progress']()?.daysToGoal).toBeNull();
    });
    it.each([null, FIXTURE_RECENT_MEASUREMENT])('summarizes last goal without inventing a missing end measurement', end => {
        ...
        expect(root.textContent).toContain('WAIST_HISTORY.LAST_GOAL');
    });
});
