...
const page = (totalItems = TOTAL_ITEMS): PageOf<FavoriteRecipe> => ({ data: [], page: 1, limit: 10, totalItems, totalPages: 3 });
describe('RecipeFavoritesPickerFacade', () => {
    let facade: RecipeFavoritesPickerFacade;
    const api = { getPage: vi.fn() };
    beforeEach(() => {
        api.getPage.mockReset().mockReturnValue(of(page()));
        TestBed.configureTestingModule({ providers: [RecipeFavoritesPickerFacade, { provide: FavoriteRecipeService, useValue: api }] });
        facade = TestBed.inject(RecipeFavoritesPickerFacade);
    });
    ...
    it('cancels obsolete requests so late results cannot overwrite a new search', () => {
        const old = new Subject<PageOf<FavoriteRecipe>>();
        const latest = new Subject<PageOf<FavoriteRecipe>>();
    ...
    it('unsubscribes an in-flight request when the dialog scope is destroyed', () => {
        const request = new Subject<PageOf<FavoriteRecipe>>();
