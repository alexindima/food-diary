...
const page = (totalItems = TOTAL_ITEMS): PageOf<FavoriteProduct> => ({ data: [], page: 1, limit: 10, totalItems, totalPages: 3 });
describe('ProductFavoritesPickerFacade', () => {
    let facade: ProductFavoritesPickerFacade;
    const api = { getPage: vi.fn() };
    beforeEach(() => {
        api.getPage.mockReset().mockReturnValue(of(page()));
        TestBed.configureTestingModule({ providers: [ProductFavoritesPickerFacade, { provide: FavoriteProductService, useValue: api }] });
        facade = TestBed.inject(ProductFavoritesPickerFacade);
    });
    ...
    it('cancels obsolete requests so late results cannot overwrite a new search', () => {
        const old = new Subject<PageOf<FavoriteProduct>>();
        const latest = new Subject<PageOf<FavoriteProduct>>();
    ...
    it('unsubscribes an in-flight request when the dialog scope is destroyed', () => {
        const request = new Subject<PageOf<FavoriteProduct>>();
