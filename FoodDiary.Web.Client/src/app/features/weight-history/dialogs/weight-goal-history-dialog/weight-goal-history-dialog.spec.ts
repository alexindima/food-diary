...
function setup({
    ...
    status?: WeightGoalHistoryItem['status'];
    current?: number | null;
    end?: number | null;
    start?: number;
    target?: number;
    ...
    component: WeightGoalHistoryDialogComponent;
    history: WritableSignal<WeightGoalHistoryItem[]>;
    close: Mock;
} {
    ...
    TestBed.configureTestingModule({
        ...
        providers: [
            ...
            { provide: WeightHistoryFacade, useValue: { weightGoalHistory: history, latestWeight: signal(current) } },
...
describe('Weight goal history dialog', () => {
    it.each([
        ...
        expect(component['goals']()[0]).toMatchObject({
            change,
            progress,
            statusKey: `WEIGHT_HISTORY.GOAL_STATUS_${status.toUpperCase()}`,
        });
    });
    it.each([
...
it('places the active goal first without mutating source order and labels historical change', () => {
    ...
    expect(entries[0].textContent).toContain('WEIGHT_HISTORY.GOAL_STATUS_ACTIVE');
    expect(entries[1].textContent).toContain('WEIGHT_HISTORY.GOAL_CHANGE_LABEL');
    expect(entries[1].querySelector('[role="progressbar"]')).toBeNull();
    expect(entries[0].querySelector('[role="progressbar"]')?.getAttribute('aria-label')).toBe('WEIGHT_HISTORY.GOAL_CARD_TITLE');
