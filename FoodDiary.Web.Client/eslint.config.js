...
const isRootProvidedInjectableDecorator = node => {
...
const localTsPlugin = {
    rules: {
        ...
        'prefer-service-decorator-for-root-singletons': {
            meta: {
                type: 'suggestion',
                docs: {
                    description: "Prefer @Service() over @Injectable({ providedIn: 'root' }) for root singleton services.",
                },
                messages: {
                    preferService:
                        "Use @Service() instead of @Injectable({ providedIn: 'root' }) for root singleton services. Keep @Injectable for constructor DI, advanced providers, and non-root scopes.",
                },
                schema: [],
            },
            create(context) {
                return {
                    Decorator(node) {
                        if (!isRootProvidedInjectableDecorator(node)) {
                            return;
                        }

                        context.report({
                            node,
                            messageId: 'preferService',
                        });
                    },
                };
            },
...
export default [
    ...
    },
    {
        ...
        rules: {
            ...
            'local/prefer-service-decorator-for-root-singletons': 'error',
