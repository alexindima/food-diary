...
import type { FdUiPieChartSegment } from 'fd-ui-kit';
...
export class AdminDashboardFacade {
    ...
    private buildLoginSegments(category: LoginSummaryCategory): FdUiPieChartSegment[] {
        return this.loginSummary()
            .map(item => ({ ...this.parseLoginSummaryKey(item.key), count: item.count }))
    ...
    private formatRelativeTime(value: string | null): string | null {
        ...
        const diffMinutes = Math.round((timestamp - Date.now()) / MS_PER_MINUTE);
        ...
        return formatter.format(Math.round(diffHours / HOURS_PER_DAY), 'day');
    }
}
