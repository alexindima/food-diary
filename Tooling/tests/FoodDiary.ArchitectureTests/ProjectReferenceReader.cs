...
internal static class ProjectReferenceReader {
    ...
    public static IReadOnlyDictionary<string, string[]> ReadTestProjectReferences() =>
        ReadTestProjectPaths()
            .ToDictionary(
                GetProjectNameFromPath,
                path => ReadProjectReferences(path).Order(StringComparer.Ordinal).ToArray(),
                StringComparer.Ordinal);
    ...
    public static IReadOnlyList<string> ReadTestProjectNames() =>
        ReadTestProjectPaths()
            .Select(GetProjectNameFromPath)
            .Order(StringComparer.Ordinal)
            .ToArray();
    ...
    private static IEnumerable<string> ReadProductionProjectPaths(string? repositoryRoot = null) =>
        RepositoryFileDiscovery.EnumerateFiles(repositoryRoot ?? ArchitectureTestPaths.RepositoryRoot, "*.csproj")
            .Where(static path => !IsTestOrTestSupportProject(path))
    ...
    private static IEnumerable<string> ReadTestProjectPaths() =>
        RepositoryFileDiscovery.EnumerateFiles(ArchitectureTestPaths.RepositoryRoot, "*.csproj")
            .Where(IsTestOrTestSupportProject)
            .Where(static path => !ArchitectureTestPaths.IsGeneratedOrBuildPath(path))
            .Order(StringComparer.Ordinal);

    private static bool IsTestOrTestSupportProject(string path) =>
        path.Contains($"{Path.DirectorySeparatorChar}tests{Path.DirectorySeparatorChar}", StringComparison.OrdinalIgnoreCase)
        || string.Equals(Path.GetFileName(path), "FoodDiary.Testing.csproj", StringComparison.Ordinal);
