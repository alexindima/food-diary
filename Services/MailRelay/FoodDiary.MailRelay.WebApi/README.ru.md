...
SMTP-доставка имеет явную гарантию **at least once**, а не exactly once. После
принятия письма удалённым SMTP-сервером MailRelay завершает запись статуса в
PostgreSQL без отмены из-за остановки хоста. При падении процесса в неизбежном
промежутке между SMTP и БД письмо может быть отправлено повторно с тем же
детерминированным `Message-Id`; приоритет отдан отсутствию тихой потери, а
стабильный идентификатор позволяет сопоставлять редкие дубли.
