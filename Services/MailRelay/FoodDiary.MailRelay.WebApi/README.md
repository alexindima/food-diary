...
- `Services/MailRelay/FoodDiary.MailRelay.WebApi`: host, config, Docker runtime, health checks, top-level composition.
- `Services/MailRelay/FoodDiary.MailRelay.Domain`: relay domain concepts and rules as they are extracted from application/infrastructure flows.
- `Services/MailRelay/FoodDiary.MailRelay.Presentation`: HTTP endpoints, internal request authorization, provider webhook adapters.
- `Services/MailRelay/FoodDiary.MailRelay.Application`: relay use cases, application models, queue/delivery abstractions, processing coordination.
- `Services/MailRelay/FoodDiary.MailRelay.Infrastructure`: PostgreSQL queue state, transactional outbox, RabbitMQ, delivery transports, DKIM, telemetry registration.
...
- `mailrelay-db-init` runs `Services/MailRelay/FoodDiary.MailRelay.Initializer` to create or update the relay schema before the app starts
- `mail-relay` runs as a separate application container built from `Services/MailRelay/FoodDiary.MailRelay.WebApi/Dockerfile`
...
```powershell
dotnet build Services/MailRelay/FoodDiary.MailRelay.WebApi/FoodDiary.MailRelay.WebApi.csproj
dotnet run --project Services/MailRelay/FoodDiary.MailRelay.WebApi
dotnet run --project Services/MailRelay/FoodDiary.MailRelay.Initializer -- status
dotnet run --project Services/MailRelay/FoodDiary.MailRelay.Initializer -- update
```

For local development, keep the database connection string in WebApi user secrets:

```powershell
dotnet user-secrets set "ConnectionStrings:DefaultConnection" "Host=localhost;Database=fooddiary_mailrelay;Username=postgres;Password=..." --project Services/MailRelay/FoodDiary.MailRelay.WebApi
```

The project launch profile sets `Development`, so `dotnet run --project Services/MailRelay/FoodDiary.MailRelay.WebApi` reads user secrets. If running the built `.exe` directly from `bin`, set `ASPNETCORE_ENVIRONMENT=Development` or pass the connection string through environment variables.
