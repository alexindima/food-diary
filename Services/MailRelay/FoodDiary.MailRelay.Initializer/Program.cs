...
if (string.IsNullOrWhiteSpace(connectionString)) {
    Console.Error.WriteLine(
        "MailRelay initializer failed: DefaultConnection is not configured. Pass --connection-string, set ConnectionStrings__DefaultConnection, set FOODDIARY_ConnectionStrings__DefaultConnection, set MAILRELAY_ConnectionStrings__DefaultConnection, or provide appsettings in Services/MailRelay/FoodDiary.MailRelay.WebApi.");
    return 1;
}
...
static void PrintUsage() {
    Console.WriteLine("""
Usage:
  dotnet run --project Services/MailRelay/FoodDiary.MailRelay.Initializer -- <command> [--connection-string "<value>"]
...
Examples:
  ...
  dotnet run --project Services/MailRelay/FoodDiary.MailRelay.Initializer -- update --connection-string "Host=..."
""");
}
