...
public sealed class MailInboxReadinessConcurrencyGate(IOptions<MailInboxHttpOptions> options) : IDisposable {
    private readonly SemaphoreSlim _slots = new(
        options.Value.MaxConcurrentReadinessRequests,
        options.Value.MaxConcurrentReadinessRequests);
    private readonly TimeSpan _queueTimeout = options.Value.ReadinessQueueTimeout;
