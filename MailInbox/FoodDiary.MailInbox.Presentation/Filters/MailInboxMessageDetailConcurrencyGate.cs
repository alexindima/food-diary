...
public sealed class MailInboxMessageDetailConcurrencyGate(IOptions<MailInboxHttpOptions> options) : IDisposable {
    private readonly SemaphoreSlim _slots = new(
        options.Value.MaxConcurrentMessageDetailRequests,
        options.Value.MaxConcurrentMessageDetailRequests);
    private readonly TimeSpan _queueTimeout = options.Value.MessageDetailQueueTimeout;
