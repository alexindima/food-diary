...
public sealed class NpgsqlInboundMailStoreHelperTests {
    ...
    public void GetCategory_ReturnsExpectedCategory(string recipient, string? subject, string expectedCategory) {
        MethodInfo? method = typeof(NpgsqlInboundMailStore).GetMethod(
            "GetCategory",
            BindingFlags.NonPublic | BindingFlags.Static);

        string category = (string)method!.Invoke(null, [new[] { recipient }, subject])!;
    ...
    public void GetCategory_WhenAnyRecipientIsDmarc_ReturnsDmarcReport() {
        ...
        string category = (string)method!.Invoke(null, [new[] { "admin@fooddiary.club", "dmarc@fooddiary.club" }, "Hello"])!;
    ...
    public void DeserializeRecipients_WhenJsonIsNull_ReturnsEmptyList() {
        ...
        IReadOnlyList<string> recipients = Assert.IsAssignableFrom<IReadOnlyList<string>>(
    ...
    public void AddWithNullableValue_WhenValueIsBlank_AddsDbNull(string? value) {
        ...
        MethodInfo method = GetNpgsqlExtensionMethod("AddWithNullableValue");
    ...
    private static MethodInfo GetNpgsqlExtensionMethod(string name) {
        Type? type = typeof(NpgsqlInboundMailStore).Assembly.GetType("FoodDiary.MailInbox.Infrastructure.Services.NpgsqlExtensions");
