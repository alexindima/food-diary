...
if ([int]$evaluation.caseCount -lt 60) { throw 'SQL context evaluation must contain at least 60 representative cases.' }
if ($null -eq $evaluation.switchReady) { throw 'SQL context evaluation did not report switch readiness.' }
if ($null -eq $evaluation.switchCriteria) { throw 'SQL context evaluation did not report switch criteria.' }
...
Write-Host "LLM Wiki SQL context evaluation passed: switchReady=$($evaluation.switchReady), top1=$($evaluation.metrics.top1Count)/$($evaluation.caseCount), top10=$($evaluation.metrics.top10Count)/$($evaluation.caseCount), MRR=$($evaluation.metrics.meanReciprocalRank), SQL p95=$($evaluation.metrics.p95SqlDurationMs)ms."
