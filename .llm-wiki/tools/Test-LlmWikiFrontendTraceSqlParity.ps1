...
if ($sqlEndToEndAverage -gt ($jsonEndToEndAverage + 250)) {
    throw "SQLite frontend trace regressed beyond the 250ms safety envelope: SQL=${sqlEndToEndAverage}ms, JSON=${jsonEndToEndAverage}ms."
}
if ($sqlEndToEndAverage -gt ($jsonEndToEndAverage - 100)) {
    throw "SQLite frontend trace did not preserve the required 100ms average improvement: SQL=${sqlEndToEndAverage}ms, JSON=${jsonEndToEndAverage}ms."
}
