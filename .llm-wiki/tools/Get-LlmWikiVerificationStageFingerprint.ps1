...
function Test-RelevantPath([string]$Path) {
    ...
    switch ($Stage) {
        ...
        'affected smoke:read-only-guard' { return $normalized -match $smokeInfrastructure -or $normalized -match '^\.llm-wiki/tools/(?:Invoke-LlmWikiReadOnlyTool|Find-LlmWikiIntentOwnership|Test-LlmWikiIntentOwnership|Test-LlmWikiReadOnlyGuard|Test-LlmWikiReadOnlyRetrievalContracts)\.ps1$' }
