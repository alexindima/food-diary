...
function Test-RelevantPath([string]$Path) {
    ...
    switch ($Stage) {
        ...
        'affected smoke:code-graph' { return $normalized -match '^\.llm-wiki/tools/(?:(?:code-graph|code-graph-path-layout|code-graph-maintenance|wiki-source-maintenance|code-graph-performance\.test|Test-LlmWikiRankingPathLayout)\.mjs|(?:Invoke-LlmWikiSelfMaintenance|Manage-LlmWikiCodeGraph|Measure-LlmWikiCodeGraph|Test-LlmWikiCodeGraph|Get-LlmWikiGraphResearch|Invoke-LlmWikiAffectedSmoke|Get-LlmWikiVerificationStageFingerprint)\.ps1)$' }
