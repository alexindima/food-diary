...
function Test-RelevantPath([string]$Path) {
    ...
    switch ($Stage) {
        ...
        'affected smoke:adaptive-routing' { return $normalized -match $smokeInfrastructure -or $normalized -match '^\.llm-wiki/(?:tools/(?:Get-LlmWikiAdaptiveWorkflow|Start-LlmWikiDevelopment|Get-LlmWikiSolutionComparison|Test-LlmWikiAdaptiveWorkflow)\.ps1|policies/experience-policies\.json|workflows/developer-experience\.md)$' }
        ...
        'affected smoke:context-bundle' { return $normalized -match $smokeInfrastructure -or $normalized -match '^\.llm-wiki/tools/(?:Find-LlmWikiContext|Manage-LlmWikiContextBundle|LlmWikiQueryCache|Test-LlmWikiContextCache)\.ps1$' }
        'affected smoke:verification-cache' { return $normalized -match $smokeInfrastructure -or $normalized -match '^\.llm-wiki/tools/(?:Manage-LlmWikiVerificationCache|Test-LlmWikiVerificationCache|Invoke-LlmWikiFullVerification)\.ps1$' }
        'affected smoke:verification-receipts' { return $normalized -match $smokeInfrastructure -or $normalized -match '^\.llm-wiki/tools/(?:LlmWikiVerificationReceipts|Manage-LlmWikiVerificationReceipts|Test-LlmWikiVerificationReceipts)\.ps1$' }
