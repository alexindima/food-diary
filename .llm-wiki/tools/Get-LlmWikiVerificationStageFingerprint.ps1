...
function Test-RelevantPath([string]$Path) {
    ...
    switch ($Stage) {
        ...
        'affected smoke:implementation-plan' { return $normalized -match '^\.llm-wiki/tools/(?:Get-LlmWikiImplementationPlan|Test-LlmWikiImplementationPlan|Invoke-LlmWikiAffectedSmoke|Get-LlmWikiVerificationStageFingerprint)\.ps1$' }
