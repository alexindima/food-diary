...
function Test-RelevantPath([string]$Path) {
    ...
    switch ($Stage) {
        ...
        'affected smoke:verification-cache' { return $normalized -match $smokeInfrastructure -or $normalized -match '^\.llm-wiki/tools/(?:Manage-LlmWikiVerificationCache|Test-LlmWikiVerificationCache|Invoke-LlmWikiFullVerification|Write-LlmWikiIndexVerificationReceipt)\.ps1$' }
