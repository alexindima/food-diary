...
if ($profile -eq 'visual-ui-change') {
    Add-Stage 'change-review' 'Confirm the actual diff stays inside the visual owner slice and derive focused checks.' "./.llm-wiki/wiki.ps1 diff; ./.llm-wiki/wiki.ps1 test-plan -Intent '$escapedObjective'; ./.llm-wiki/wiki.ps1 verify-fast" $true 'Actual paths remain visual-only and fast Wiki validation passes without unrelated index regeneration.'
    ...
    Add-Stage 'browser-evidence' 'Verify the changed rendering at representative desktop and mobile viewports.' '# run focused browser QA and capture visual evidence' $true 'Browser evidence proves layout, responsive behavior, and the absence of console errors.'
} else {
