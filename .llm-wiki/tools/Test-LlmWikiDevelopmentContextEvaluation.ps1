...
if (-not $evaluation.passed) {
    throw "Development-context evaluation missed its thresholds: sqlite=$($evaluation.metrics.sqlitePrimaryRate), scope=$($evaluation.metrics.scopeRecallRate), complete=$($evaluation.metrics.completeBundleRate), checks=$($evaluation.metrics.focusedChecksRate), p95=$($evaluation.metrics.p95DurationMilliseconds)ms."
}
Write-Host "LLM Wiki development-context evaluation passed: cases=$($evaluation.caseCount), sqlite=$($evaluation.metrics.sqlitePrimaryRate), scope=$($evaluation.metrics.scopeRecallRate), complete=$($evaluation.metrics.completeBundleRate), checks=$($evaluation.metrics.focusedChecksRate), averageScope=$($evaluation.metrics.averageExpandedScopePaths), p95=$($evaluation.metrics.p95DurationMilliseconds)ms, maxCompact=$($evaluation.metrics.maximumCompactCharacters) chars."
