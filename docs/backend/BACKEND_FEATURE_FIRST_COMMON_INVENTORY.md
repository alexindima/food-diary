...
Feature-specific error factories should move incrementally to feature folders while preserving the existing `Errors.<Feature>` facade as the compatibility API for existing call sites. Product, recipe, favorite meal, favorite product, favorite recipe, recipe comment, shopping list, content report, meal plan, daily advice, cycle, cycle day, lesson, image, fasting, wearable, USDA, weight entry, waist entry, hydration entry, and exercise errors now live in their feature `Common/*Errors.cs` files, with `Errors.<Feature>` delegating to those feature-owned implementations.
