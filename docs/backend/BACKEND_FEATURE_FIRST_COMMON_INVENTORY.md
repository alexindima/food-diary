...
Feature-specific error factories should move incrementally to feature folders while preserving the existing `Errors.<Feature>` facade as the compatibility API for existing call sites. Product, weight entry, waist entry, hydration entry, exercise, favorite product, favorite recipe, recipe comment, shopping list, meal plan, daily advice, cycle, cycle day, lesson, and image errors now live in their feature `Common/*Errors.cs` files, with `Errors.<Feature>` delegating to those feature-owned implementations.
