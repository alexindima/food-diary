...
| Identity | login/register/restore workflows, refresh sessions, login events and email templates | authentication commands, token/session lifecycle and email-template administration under `Modules/Identity/Application`, preserving the `FoodDiary.Application.Identity` assembly | Users authentication capabilities, central persistence seams, Notifications, MailRelay and external identity validators |
...
| Lessons | nutrition lessons and user lesson progress | isolated application capability in `FoodDiary.Application.Lessons`, lesson queries/progress command, `ILessonAdministrationService` | Users access; Admin invokes the owner administration capability and consumes admin projections |
