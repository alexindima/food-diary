...
| WeeklyGoals | `WeeklyGoal`; aggregate/id/enum remain in central Domain as a documented CLR and EF compatibility seam | commands, queries, progress calculation, reminder processing, ports, contracts, and persistence under `Modules/WeeklyGoals` | Meals through `IMealActivityReadService`; Notifications and shared unit of work through central application contracts; JobManager as scheduler adapter |
