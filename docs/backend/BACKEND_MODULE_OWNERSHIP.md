...
AI, Hydration, TDEE and Weekly Check-In use dedicated projection contracts from `FoodDiary.Application.Abstractions/Users`. Their application services receive only the fields required by their calculation or display behavior; they do not receive the `User` aggregate. Weekly Check-In performs access validation separately through `ICurrentUserAccessService`.
