...
| Export | synchronous privacy-sensitive diary/cycle read composition; owns no aggregate, retained file, or persistence model | export contracts and bounded CSV/file generation under `Modules/Export/Application`; legacy assembly/CLR namespaces and HTTP contract remain stable | Meals and Cycles semantic read services; Users current-access/password verification; central PDF/resource adapters and Presentation transport |
...
| Wearables | provider connections and synchronization entries in `Modules/Wearables/Domain` | module-owned ports/use cases under `Modules/Wearables/Application`; persistence and credential protection under `Modules/Wearables/Infrastructure` | Central Users identity and database lifecycle; provider client adapters in `FoodDiary.Integrations` |
...
Images owns `ImageAsset`, upload validation, cleanup policy and the durable object-deletion outbox. Its Application, ports, image-asset EF configuration, and repository adapters are physically isolated under `Modules/Images`; the Application assembly keeps the legacy `FoodDiary.Application.Images` identity. `ImageAsset` remains in central Domain because `MealAiSession` has a legacy EF navigation to it, while the outbox persistence type/configuration remains central because it participates in the shared outbox engine and migration model. Other modules use `IImageAssetAccessService` and `IImageAssetCleanupService`; they must not acquire image repositories. AI image analysis resolves and validates ownership through the Images capability rather than loading `ImageAsset` persistence directly.
