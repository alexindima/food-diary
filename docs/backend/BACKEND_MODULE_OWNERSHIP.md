...
| Identity | login/register/restore workflows, refresh sessions, login events and email templates | authentication commands, token/session lifecycle and email-template administration under `Modules/Identity/Application`, using the `FoodDiary.Modules.Identity.Application` assembly | Users authentication capabilities, central persistence seams, Notifications, MailRelay and external identity validators |
...
| RecipeCommunity | recipe comments and likes | comment commands/read service and like set-state/status grouped in `Modules/RecipeCommunity/Application` | Recipes access API, Users, Notifications writer |
| Body Metrics | `WeightEntry` and `WaistEntry` measurements plus their IDs and invariants under `Modules/BodyMetrics/Domain`; legacy CLR namespaces and forward User navigation remain, while inverse User measurement collections are intentionally absent | measurement commands and read capabilities under `Modules/BodyMetrics/Application`; repository ports in Application/Abstractions and persistence under Infrastructure | User/UserId compatibility and goal lifecycle remain central; Dashboard, Statistics, Weekly Check-In and TDEE are read-only projection consumers; central Dashboard, Users and cleanup projections retain their approved seams |
| Hydration | `HydrationEntry`, its ID/invariants, and hydration totals; domain types live in `Modules/Hydration/Domain` with stable CLR names and no inverse User collection | hydration commands and goal capability in `Modules/Hydration/Application`; stable `IHydrationEntryReadService` in Contracts | Central `User`/`UserId` one-way compatibility types; Dashboard and Weekly Check-In as read-only projection consumers |
...
| Export | synchronous privacy-sensitive diary/cycle read composition; owns no aggregate, retained file, or persistence model | export contracts and bounded CSV/file generation under `Modules/Export/Application`; PDF rendering, localized report text and safe image HTTP client under `Modules/Export/Infrastructure`; application identity and HTTP contract remain stable | Meals and Cycles semantic read services; Users current-access/password verification; Presentation transport |
| MealPlanning | meal plans plus shopping lists, items and provenance | MealPlans and ShoppingLists use cases grouped in `Modules/MealPlanning/Application`; `IMealPlanReadService`, `IShoppingListCreationService`, `IShoppingListReadService` | Users access, Recipes/Product projections |
| Wearables | provider connections and synchronization entries in `Modules/Wearables/Domain` | module-owned ports/use cases, persistence, credential protection and provider clients under `Modules/Wearables` | Central Users identity and database lifecycle |
...
| Lessons | nutrition lessons and user lesson progress | isolated application capability in `FoodDiary.Modules.Lessons.Application`, lesson queries/progress command, `ILessonAdministrationService` | Users access; Admin invokes the owner administration capability and consumes admin projections |
...
| OpenFoodFacts Cache | cached external product documents and refresh lifecycle | `Modules/OpenFoodFacts`: cached product-search contract, application orchestration, cache entity, persistence and provider adapter | Products consumes Contracts only; central Infrastructure retains DbContext/migrations/snapshot |
...
Users and Identity are separate collaborating modules around one identity lifecycle. Users owns the `User`, `Role`, `UserRole` and `UserRoleAuditEvent` aggregates/state, including credential verification and account lifecycle mutation. Identity is physically isolated in `FoodDiary.Modules.Identity.Application` and owns login/register/restore orchestration, refresh-token sessions, login-event history and application email-template administration. Authentication and Email remain logical feature areas inside that assembly.
...
Images EF/outbox configuration belongs to `Modules/Images/PersistenceModel`. Favorite relationship configurations live in `Modules/Favorites/PersistenceModel/Configurations`, with repositories separated by slice under module Infrastructure. Shared context/migrations remain central.
...
Dashboard is a composed read model. Its production infrastructure adapter may query the shared database to build an optimized projection, but it owns none of the contributing aggregates and exposes no mutation capability. The application fallback read path is likewise projection-only. Statistics is physically isolated in `Modules/Statistics/Application` and composes Dashboard and Body Metrics projections without owning their aggregates, persistence, or a separate contracts surface. Weekly Check-In, TDEE and Gamification are calculation/read modules: they may consume Body Metrics, Hydration, Exercises, Meal Diary and Dashboard statistics projections, but must not acquire their aggregate or write repositories.
...
Body Metrics and Exercises configurations live in their module-owned `Infrastructure/Model` projects. Hydration and Cycles configurations live in their module-owned `Infrastructure/Model` projects and are applied by the central `FoodDiaryDbContext`. Architecture tests protect both application repository boundaries and configuration placement.

## Planning, Wearables and Marketing boundaries

MealPlanning is the shared physical logical-module boundary under `Modules/MealPlanning` for the separate MealPlans and ShoppingLists logical owners. Meal Plans reads its own aggregate and submits a `ShoppingListCreationRequest` through `IShoppingListCreationService`; Shopping Lists alone constructs and persists the `ShoppingList` aggregate. Grouping the related planning workflow in one assembly does not transfer write ownership between its aggregates. Both aggregate graphs, their IDs, ShoppingLists events and source enum live in the module Domain with stable CLR namespaces; their EF configurations remain separated under `Modules/MealPlanning/PersistenceModel/Configurations/ShoppingLists` and `MealPlans`. ShoppingList keeps a one-way relationship to Users-owned User through scalar `UserId` and `ShoppingList.User`; the schema-equivalent mapping has no inverse CLR navigation on User. See `Modules/MealPlanning/AGENTS.md`.

Wearables owns provider connections and synchronization history under `Modules/Wearables`. Application abstractions, use cases, domain, persistence model, infrastructure, Fitbit HTTP client and typed options are module-owned; the application project preserves the `FoodDiary.Application.Wearables` assembly identity. Complete registration flows through infrastructure `AddWearablesModule` plus its explicit provider registration, while `FoodDiary.Application.Runtime` does not aggregate it. OAuth-state and token protection remain inside module infrastructure. The shared `FoodDiaryDbContext`, historical migrations, and snapshot remain central, and the context applies the module persistence model through `ApplyWearablesPersistenceModel`. No recurring background wearable sync is registered today; synchronization remains an explicit application command.

Marketing owns attribution events and conversion state under `Modules/Marketing`. Its application assembly preserves the `FoodDiary.Application.Marketing` identity and registers handlers and services through `AddMarketingApplication`; module Infrastructure exposes the complete `AddMarketingModule` composition facade. Authentication records attribution through the command API, Billing emits premium conversion through its consumer-owned `IBillingMarketingConversionRecorder` port, and reporting uses the attribution summary read service. Repository ports are module-owned, while the EF configuration and adapter live in module Infrastructure. The shared `FoodDiaryDbContext`, historical migrations, and model snapshot remain central.
...
Remaining central technical adapters are limited to genuinely cross-module infrastructure. Extracted USDA domain, persistence, provider HTTP and in-memory detail caching live under `Modules/Usda`; Products Domain references USDA Domain one-way for the accepted scoring and EF navigation seam described in ADR 0027.

Executable hosts, shared Presentation, Initializer and JobManager may not inject repository contracts. They invoke application capabilities or compose explicit module/shared adapters. This is enforced across all primary backend adapter projects by a single architecture guardrail.
