...
| Export | synchronous privacy-sensitive diary/cycle read composition; owns no aggregate, retained file, or persistence model | export contracts and bounded CSV/file generation under `Modules/Export/Application`; PDF rendering and safe image HTTP client under `Modules/Export/Infrastructure`; application identity and HTTP contract remain stable | Meals and Cycles semantic read services; Users current-access/password verification; Resources localization and Presentation transport |
