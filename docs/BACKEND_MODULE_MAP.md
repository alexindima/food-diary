...
| Application runtime | `Shared/FoodDiary.Application.Runtime` | Mediator pipeline behaviors, transaction boundary, post-commit queue registration | Feature handlers, validators, business services, module aggregation |
| Shared audit adapter | `Shared/FoodDiary.Audit.Infrastructure` | Audit logging and read/write adapters over the shared persistence session; explicit host registration | Module business rules, persistence coordination, migrations |
| Shared email adapter | `Shared/FoodDiary.Email.Infrastructure` | Email outbox enqueue/processing and email replay stream; explicit host registration | Email templates, token protocols, persistence coordination, migrations |
...
| BodyMetrics domain | `Modules/BodyMetrics/Domain` | Weight/waist measurement entries, IDs, and invariants with module-owned folder namespaces and a scalar `UserId` dependency on Users Domain.Contracts; User-owned goal lifecycle belongs to Users Domain | Reverse User measurement navigations, goal lifecycle, application orchestration, EF mappings, transport |
| BodyMetrics application ports | `Modules/BodyMetrics/Application.Abstractions` | Weight/waist write and read-model repository ports, errors, and internal projections; public read capabilities live in Contracts | EF, HTTP transport, provider adapters |
| BodyMetrics use cases | `Modules/BodyMetrics/Application` | Weight/waist commands, queries, handlers, validation and shared read services in FoodDiary.Modules.BodyMetrics.Application | Persistence implementations and HTTP transport |
| BodyMetrics persistence model | `Modules/BodyMetrics/PersistenceModel` | Entry EF configurations and explicit model-builder registration | Central DbContext, migrations, snapshot, and User-owned goal mappings |
...
| Hydration persistence model | `Modules/Hydration/PersistenceModel` | Hydration EF configuration and model-builder registration seam | Shared `DbContext`, migrations, repository behavior |
...
| WeeklyGoals persistence model | `Modules/WeeklyGoals/PersistenceModel` | WeeklyGoals EF configuration and model-builder registration seam | Shared `DbContext`, migrations, repository behavior |
...
| Wearables persistence model | `Modules/Wearables/PersistenceModel` | Wearables EF configurations and model-builder registration seam | Shared `DbContext`, migrations, repository behavior |
...
| Lessons domain | `Modules/Lessons/Domain` | `NutritionLesson`, `UserLessonProgress`, internal progress ID and invariants with preserved CLR identity | Application orchestration, EF mapping, transport |
| Lessons scalar contracts | `Modules/Lessons/Domain.Contracts` | NutritionLessonId, LessonCategory and LessonDifficulty | Aggregates, repositories and application implementations |
...
| Lessons persistence model | `Modules/Lessons/PersistenceModel` | Lesson EF configurations and model-builder registration seam | Shared `DbContext`, migrations, repository behavior |
...
| Images persistence model | `Modules/Images/PersistenceModel` | `ImageAsset` EF configuration and model-builder seam | Shared DbContext, migrations, outbox engine |
...
| Billing use cases | `Modules/Billing/Application` | Billing commands, queries, validators, entitlement, renewal and webhook orchestration with preserved `FoodDiary.Modules.Billing.Application` assembly identity | Persistence, provider adapters, HTTP DTOs |
| Billing persistence model | `Modules/Billing/PersistenceModel` | Billing EF configurations and explicit model-builder registration | Shared `DbContext`, migrations, repository behavior |
...
| Marketing persistence model | `Modules/Marketing/PersistenceModel` | Attribution EF configuration and central model-builder seam | Shared `DbContext`, migrations, repository behavior |
...
| Persistence/technical implementations | `FoodDiary.Infrastructure` | Full DbContext/migrations/snapshot and cross-module EF mappings; runtime engines live in Shared/FoodDiary.Persistence.Runtime, authentication binding/storage in Shared/FoodDiary.Authentication.Infrastructure | HTTP controllers, host startup, external provider orchestration |
...
The same registration owns ordinary Admin SSO; shared code storage, API Redis
