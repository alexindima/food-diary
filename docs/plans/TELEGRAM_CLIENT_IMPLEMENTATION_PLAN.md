...
Дата: 2026-09-12. Статус: реализация начата. Готовность всей фичи ещё не подтверждена. Текущее состояние: `.artifacts/llm-wiki/tasks/telegram-client/`.
...
- Отдельного тестового окружения нет: подготовка отдельного тестового стенда входит в задачу.
...
| Запись еды | `Modules/Meals/Presentation/MealsController.cs` | POST с EnableIdempotency, PATCH, DELETE; AI-сессии в DTO |
...
### Отдельный тестовый стенд

Подтверждено пользователем: стенд нужно подготовить. Это самостоятельный этап до live smoke, а не предположение о наличии готовой инфраструктуры.

1. Подготовить изолированный Compose-проект `fooddiary-telegram-test` с отдельными PostgreSQL, Redis, очередями, хранилищем изображений и постоянным Data Protection key ring. Использовать пустую базу и синтетические аккаунты; применить штатный initializer. Не копировать production данные.
2. Подготовить отдельный файл конфигурации вне Git и шаблон без значений секретов. В текущем `docker-compose.yml` сервисы явно читают `.env`, а API содержит production MailInbox URL: одного `--env-file` и нового имени проекта недостаточно. Отдельная конфигурация должна исключать наследование этих настроек, production credentials, отправку реальных писем и фоновые production интеграции. Проверять итоговую конфигурацию без вывода секретов.
3. Собрать API, JobManager, frontend и bot из проверяемого checkout. Выделить свободные локальные порты, публиковать инфраструктурные порты только на loopback. Проверить readiness и работу frontend с API до подключения Telegram.
4. Выделить отдельный HTTPS origin для frontend и API через reverse proxy. Для локального стенда допустим HTTPS tunnel; зафиксировать конкретный адрес до регистрации callback. База, очереди и служебные интерфейсы не публикуются через tunnel. Постоянное размещение на сервере требует выбора конкретного хоста и домена.
5. Создать отдельного тестового бота через BotFather; настроить Mini App URL и разрешённый OIDC callback `/auth/telegram/callback`. Его Telegram ID должен совпадать с настройками API и bot. Секреты вводятся в локальную конфигурацию, не в чат. Не запускать два long-polling процесса с одним токеном.
6. Сначала проверить детерминированные сценарии с тестовыми внешними адаптерами. Затем отдельно выполнить настоящий Telegram smoke с тестовым AI-доступом и ограниченным расходом: регистрация без email, вход браузером, привязка, фото → дневник → отмена, вода, сводки, редактирование Mini App, перезапуск и отключение связи. Не выдавать mocked прогон за проверку живого провайдера.
7. Сохранить команды запуска/остановки, адрес стенда, проверяемую ревизию и результаты TG-01–TG-20 в runbook. Стенд готов только после проверки HTTPS, сохранности данных и key ring после перезапуска и отсутствия обращений к production интеграциям. Удаление тестовых данных оформляется отдельной явной командой с проверкой имени проекта.

Внешние зависимости для завершения этого этапа: доступ к BotFather и выбранному HTTPS адресу; для серверного размещения — конкретный хост. До их получения можно подготовить конфигурацию, сборки и локальные проверки. Текущий раздел описывает работу, а не утверждает, что стенд уже запущен.
...
Подготовь отдельный тестовый стенд по разделу 12: изолированные данные и конфигурация,
отдельный бот, HTTPS origin, штатные миграции и воспроизводимый запуск.
Не считай существующий root Compose изолированным только из-за нового имени проекта.
