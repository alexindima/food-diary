...
## ☁️ Настройка хранения изображений (S3)

- В `FoodDiary.Web.Api/appsettings*.json` заполните секцию `S3` (`AccessKeyId`, `SecretAccessKey`, `Region`, `Bucket`). Для S3-совместимых стораджей можно указать `ServiceUrl` и `PublicBaseUrl` (например, CDN); при наличии `ServiceUrl` автоматически используется путь вида `/bucket/key`.
- Ограничение загрузки: 20 МБ, разрешены JPG/PNG/GIF/WebP.
- Эндпоинт `POST /api/images/upload-url` возвращает пресайн-ссылку для `PUT` в S3 и `fileUrl`, который нужно сохранять в `imageUrl` сущностей (продуктов, рецептов, приёмов пищи).
