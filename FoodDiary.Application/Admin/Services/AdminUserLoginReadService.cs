...
public sealed class AdminUserLoginReadService(IUserLoginEventReadRepository repository) : IAdminUserLoginReadService {
    public async Task<Result<PagedResponse<AdminUserLoginEventModel>>> GetEventsAsync(
        ...
        (IReadOnlyList<UserLoginEventReadModel> items, int totalItems) =
            await repository.GetPagedAsync(normalizedPage, normalizedLimit, userId, search, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<Result<IReadOnlyList<AdminUserLoginDeviceSummaryModel>>> GetSummaryAsync(
        ...
        IReadOnlyList<UserLoginDeviceSummaryModel> summary =
            await repository.GetDeviceSummaryAsync(fromUtc, toUtc, cancellationToken).ConfigureAwait(false);
