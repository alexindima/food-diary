...
internal sealed class AuthenticationUserRegistrationService(IUserRepository userRepository) : IAuthenticationUserRegistrationService {
    ...
    public Task<IReadOnlyList<Role>> EnsureRolesByNamesAsync(IReadOnlyList<string> names, CancellationToken cancellationToken = default) =>
        userRepository.EnsureRolesByNamesAsync(names, cancellationToken);
