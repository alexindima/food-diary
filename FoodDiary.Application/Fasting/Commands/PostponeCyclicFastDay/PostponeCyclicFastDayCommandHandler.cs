...
public sealed class PostponeCyclicFastDayCommandHandler(
    IFastingPlanRepository fastingPlanRepository,
    IFastingOccurrenceRepository fastingOccurrenceRepository,
    IDateTimeProvider dateTimeProvider,
    IUnitOfWork unitOfWork)
    : ICommandHandler<PostponeCyclicFastDayCommand, Result<FastingSessionModel>> {
    public async Task<Result<FastingSessionModel>> Handle(
        PostponeCyclicFastDayCommand command, CancellationToken cancellationToken) {
        ...
        if (plan.Type != FastingPlanType.Cyclic || current.Kind != FastingOccurrenceKind.FastDay) {
            return Result.Failure<FastingSessionModel>(Errors.Fasting.InvalidCyclicAction("Only an active cyclic fast day can be postponed."));
        }
        ...
        try {
            ...
            return Result.Failure<FastingSessionModel>(Errors.Fasting.InvalidCyclicAction("The cyclic fast day can only be postponed to a later date."));
        } catch (InvalidOperationException) {
            return Result.Failure<FastingSessionModel>(Errors.Fasting.InvalidCyclicAction("The current cyclic fast day cannot be postponed."));
        }
