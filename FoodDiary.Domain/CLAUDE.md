...
Complex aggregates decompose mutable state into `readonly record struct` state objects (e.g., `MealDetailsState`, `UserSecurityState`). Mutations apply via `with` expressions on the record.
