...
public class LessonsFeatureTests {
    ...
    public async Task MarkLessonRead_WhenLessonExists_Succeeds() {
        ...
        var handler = new MarkLessonReadCommandHandler(repo, repo, new FixedDateTimeProvider(), Substitute.For<ICurrentUserAccessService>(), outbox, new InlineLessonProgressTransactionRunner());
    ...
    public async Task MarkLessonRead_WhenLessonNotFound_ReturnsFailure() {
        ...
        var handler = new MarkLessonReadCommandHandler(repo, repo, new FixedDateTimeProvider(), Substitute.For<ICurrentUserAccessService>(), Substitute.For<IAchievementEvaluationOutbox>(), new InlineLessonProgressTransactionRunner());
    ...
    public async Task MarkLessonRead_WithNullUserId_ReturnsFailure() {
        var handler = new MarkLessonReadCommandHandler(
            ...
            Substitute.For<IAchievementEvaluationOutbox>(), new InlineLessonProgressTransactionRunner());
    ...
    private static GetLessonsQueryHandler CreateGetLessonsHandler(
        INutritionLessonRepository repository) =>
        new(repository, Substitute.For<ICurrentUserAccessService>());
    ...
    [ExcludeFromCodeCoverage]
    private sealed class InlineLessonProgressTransactionRunner : ILessonProgressTransactionRunner {
        public Task<T> ExecuteSerializedAsync<T>(UserId userId, NutritionLessonId lessonId,
            Func<CancellationToken, Task<T>> operation, CancellationToken cancellationToken = default) => operation(cancellationToken);
    }
}
