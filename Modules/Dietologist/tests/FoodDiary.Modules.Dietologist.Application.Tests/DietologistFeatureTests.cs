...
public partial class DietologistFeatureTests {
    ...
    public async Task DietologistUserContextService_FindByIdAsync_ReturnsProfile() {
        ...
        IUserDietologistProfileReadService userLookupService = Substitute.For<IUserDietologistProfileReadService>();
    ...
    private static GetInvitationForCurrentUserQueryHandler CreateGetInvitationForCurrentUserHandler(
        ...
        IDietologistUserContextService users = userContextService ?? new InMemoryUserRepository();
        return new(invitationRepository ?? new InMemoryInvitationRepository(), users, TimeProvider.System, users);
    }
    ...
    private static GetMyDietologistQueryHandler CreateGetMyDietologistHandler(
        ...
        new(invitationRepository ?? new InMemoryInvitationRepository(), currentUserAccessService ?? new InMemoryUserRepository());
    ...
    private static IDietologistInvitationReadService CreateDietologistInvitationReadService(
        ...
        new DietologistInvitationReadService(invitationRepository, currentUserAccessService);
    ...
    private static GetClientGoalsQueryHandler CreateGetClientGoalsHandler(
        IDietologistInvitationReadModelRepository? invitationRepository = null,
        InMemoryUserRepository? userRepository = null) =>
        new(invitationRepository ?? new InMemoryInvitationRepository(),
            userRepository ?? new InMemoryUserRepository(),
            userRepository ?? new InMemoryUserRepository());
    ...
    private static GetMyRecommendationsQueryHandler CreateGetMyRecommendationsHandler(
        ...
        ICurrentUserAccessService? currentUserAccessService = null) =>
        new(recommendationRepository ?? new InMemoryRecommendationRepository(),
            currentUserAccessService ?? new InMemoryUserRepository());
    ...
    public async Task DietologistInvitationReadService_GetForCurrentUser_WhenEmailLookupFails_ReturnsFailure() {
        var service = new GetInvitationForCurrentUserQueryHandler(new InMemoryInvitationRepository(), new InMemoryUserRepository(), TimeProvider.System, new InMemoryUserRepository());

        Result<DietologistInvitationForCurrentUserModel> result = await service.Handle(new GetInvitationForCurrentUserQuery(UserId.New().Value, Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task DietologistInvitationReadService_GetByToken_WithEmptyInvitationId_ReturnsFailure() {
        var service = new GetInvitationByTokenQueryHandler(new InMemoryInvitationRepository(), new InMemoryUserRepository(), TimeProvider.System, new InMemoryUserRepository());

        Result<InvitationModel> result = await service.Handle(new GetInvitationByTokenQuery(UserId.New().Value, Guid.Empty), CancellationToken.None);
    ...
    private sealed class InMemoryInvitationRepository : IDietologistInvitationRepository {
        ...
        public static DietologistInvitationReadModel? ToReadModel(DietologistInvitation? invitation) =>
            invitation is null
                ? null
                : new(
                invitation.Id.Value,
                ...
                GetInvitationProfiles(invitation).Client?.Email ?? "client@example.com",
                GetInvitationProfiles(invitation).Client?.FirstName,
                GetInvitationProfiles(invitation).Client?.LastName,
                GetInvitationProfiles(invitation).Client?.ProfileImage,
                GetInvitationProfiles(invitation).Client?.BirthDate,
                GetInvitationProfiles(invitation).Client?.Gender,
                GetInvitationProfiles(invitation).Client?.HeightCm,
                GetInvitationProfiles(invitation).Client?.ActivityLevel ?? ActivityLevel.Moderate,
                GetInvitationProfiles(invitation).Dietologist?.Email,
                GetInvitationProfiles(invitation).Dietologist?.FirstName,
                GetInvitationProfiles(invitation).Dietologist?.LastName,
                invitation.Status,
    ...
    [ExcludeFromCodeCoverage]
    private sealed class InMemoryUserRepository : IUserRepository, ICurrentUserAccessService, IDietologistUserContextService, IUserDietologistProfileReadService {
    ...
    [ExcludeFromCodeCoverage]
    private sealed class SequenceUserRepository(params User?[] users) : IUserRepository, IDietologistUserContextService, IUserDietologistProfileReadService {
