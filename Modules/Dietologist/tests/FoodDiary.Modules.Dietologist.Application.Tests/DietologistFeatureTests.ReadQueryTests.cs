...
using FoodDiary.Modules.Dashboard.Application.Queries.GetDietologistClientDashboard;
...
public partial class DietologistFeatureTests {
    ...
    public async Task InvitationReadService_AtExactExpiry_UsesExpiredBoundaryConsistently() {
        ...
        var service = new GetInvitationByTokenQueryHandler(invitationRepository, userRepository, new FixedTimeProvider(utcNow), userRepository);

        Result<InvitationModel> tokenResult = await service.Handle(new GetInvitationByTokenQuery(dietologistId.Value, invitation.Id.Value), CancellationToken.None);
        DietologistInvitationForCurrentUserModel currentUserModel = ResultAssert.Success(
            await new GetInvitationForCurrentUserQueryHandler(invitationRepository, userRepository, new FixedTimeProvider(utcNow), userRepository).Handle(new GetInvitationForCurrentUserQuery(dietologistId.Value, invitation.Id.Value), CancellationToken.None));
    ...
    public async Task GetClientGoals_WhenDietologistEmailLookupFails_ReturnsFailure() {
        var service = new GetClientGoalsQueryHandler(new InMemoryInvitationRepository(), new InMemoryUserRepository(), new InMemoryUserRepository());

        Result<UserModel> result = await service.Handle(new GetClientGoalsQuery(UserId.New().Value, Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task GetClientGoals_WithEmptyClientId_ReturnsFailure() {
        ...
        var userRepo = new InMemoryUserRepository();
        userRepo.Seed(CreateUser(dietologistId, "diet@example.com"));
        var service = new GetClientGoalsQueryHandler(new InMemoryInvitationRepository(), userRepo, userRepo);

        Result<UserModel> result = await service.Handle(new GetClientGoalsQuery(dietologistId.Value, Guid.Empty), CancellationToken.None);
    ...
    public async Task GetMyRecommendations_WhenCurrentUserAccessFails_ReturnsFailure() {
        var service = new GetMyRecommendationsQueryHandler(new InMemoryRecommendationRepository(), new InMemoryUserRepository());

        Result<IReadOnlyList<RecommendationModel>> result = await service.Handle(new GetMyRecommendationsQuery(UserId.New().Value), CancellationToken.None);
    ...
    public async Task GetRecommendationsForClient_WhenDietologistAccessFails_ReturnsFailure() {
        var service = new GetRecommendationsForClientQueryHandler(new InMemoryInvitationRepository(), new InMemoryRecommendationRepository(), new InMemoryUserRepository());

        Result<IReadOnlyList<RecommendationModel>> result = await service.Handle(new GetRecommendationsForClientQuery(UserId.New().Value, Guid.NewGuid()), CancellationToken.None);
    ...
    public async Task GetRecommendationsForClient_WithEmptyClientId_ReturnsFailure() {
        ...
        var userRepo = new InMemoryUserRepository();
        userRepo.Seed(CreateUser(dietologistId, "diet@example.com"));
        var service = new GetRecommendationsForClientQueryHandler(new InMemoryInvitationRepository(), new InMemoryRecommendationRepository(), userRepo);

        Result<IReadOnlyList<RecommendationModel>> result = await service.Handle(new GetRecommendationsForClientQuery(dietologistId.Value, Guid.Empty), CancellationToken.None);
    ...
    public async Task GetMyDietologist_WhenCurrentUserAccessFails_ReturnsFailure() {
        var service = new GetMyDietologistQueryHandler(new InMemoryInvitationRepository(), new InMemoryUserRepository());

        Result<DietologistInfoModel?> result = await service.Handle(new GetMyDietologistQuery(UserId.New().Value), CancellationToken.None);
    ...
    public async Task GetMyClients_WhenCurrentUserAccessFails_ReturnsFailure() {
        var service = new DietologistInvitationReadService(new InMemoryInvitationRepository(), new InMemoryUserRepository());
    ...
    public async Task ProfileDietologistReadService_WhenRelationshipExists_MapsConsumerOwnedProjection() {
        ...
        var service = new DietologistInvitationReadService(invitationRepository, users);
