...
MailInbox integration registers the single SendBugAcknowledgementsCommand handler and mediator for the background worker. Use idempotent handler registration so composition with AddAdminApplication does not duplicate it; JobManager still does not register the whole Admin application.
