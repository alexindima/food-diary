...
public static class AdminRetentionHttpMappings {
    public static GetAdminRetentionQuery ToQuery(this GetAdminRetentionHttpQuery query) => new(query.From, query.To, query.CohortFrom, query.CohortTo);
    public static AdminRetentionReportHttpResponse ToHttpResponse(this AdminRetentionReport report) =>
        new(report.FromUtc, report.ToUtc, report.AsOfUtc, report.ActiveUsersInPeriod,
            ...
            report.ActivityByDay.Select(row => new AdminRetentionDayHttpResponse(row.Date, row.ActiveUsers, row.MealEntries)).ToList(),
            report.MealEntriesInPeriod, report.CohortFromUtc, report.CohortToUtc);
}
