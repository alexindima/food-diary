...
internal static class LocalStatisticsCalendar {
    internal static IReadOnlyList<LocalStatisticsDay> GetDays(DateTime utcNow, string timeZoneId, int days) =>
        GetDays(utcNow, timeZoneId, days, TimeZoneInfo.FindSystemTimeZoneById);

    internal static IReadOnlyList<LocalStatisticsDay> GetDays(DateTime utcNow, string timeZoneId, int days, Func<string, TimeZoneInfo> resolveTimeZone) {
    ...
    private static DateTime StartOfDayUtc(DateOnly date, TimeZoneInfo zone) {
        DateTime local = FindFirstValidLocalTime(date.ToDateTime(TimeOnly.MinValue, DateTimeKind.Unspecified), zone.IsInvalidTime);
    ...
    internal static DateTime FindFirstValidLocalTime(DateTime local, Func<DateTime, bool> isInvalidTime) {
        // Some zones move their clocks at midnight or skip a whole civil date.
        int minutes = 0;
        while (isInvalidTime(local)) {
            if (++minutes > 2880) {
