...
public class HydrationFeatureTests {
    private static ISender CreateHydrationSender(IHydrationEntryReadModelRepository repository) =>
        RequestTestSender.Create(new ReadHydrationEntriesQueryHandler(repository), new ReadHydrationDailyTotalQueryHandler(repository), new ReadHydrationDailyTotalsQueryHandler(repository));
    ...
    public async Task GetHydrationDailyTotalQueryHandler_WhenUserIsMissing_ReturnsInvalidToken() {
        ...
        Result<HydrationDailyModel> result = await service.Handle(new GetHydrationDailyTotalQuery(user.Id.Value, DateTime.UtcNow), CancellationToken.None);
    ...
    public async Task GetHydrationDailyTotalQueryHandler_WithUnspecifiedDate_PreservesCalendarDayAsUtc() {
        ...
        IUserHydrationProfileReadService hydrationGoalService = CreateHydrationProfileService(user);
        var handler = new GetHydrationDailyTotalQueryHandler(CreateHydrationSender(repository),
    ...
    public async Task GetHydrationDailyTotalQueryHandler_WithEmptyUserId_ReturnsInvalidToken() {
        var handler = new GetHydrationDailyTotalQueryHandler(CreateHydrationSender(new RecordingHydrationEntryRepository()),
            CreateHydrationProfileService(User.Create("user@example.com", "hash")),
    ...
    public async Task GetHydrationDailyTotalQueryHandler_WithDeletedUser_ReturnsAccountDeleted() {
        ...
        var handler = new GetHydrationDailyTotalQueryHandler(CreateHydrationSender(new RecordingHydrationEntryRepository()),
            CreateHydrationProfileService(user),
            CreateCurrentUserAccessService(user));
    ...
    public async Task HydrationEntryReadService_MapsReadModelsAndForwardsDailyTotals() {
        ...
        ISender service = RequestTestSender.Create(new ReadHydrationEntriesQueryHandler(repository), new ReadHydrationDailyTotalQueryHandler(repository), new ReadHydrationDailyTotalsQueryHandler(repository));

        IReadOnlyList<HydrationEntryModel> entries = await service.Send(new ReadHydrationEntriesQuery(userId, date), CancellationToken.None);
        IReadOnlyList<(DateTime Date, int TotalMl)> totals = await service.Send(new ReadHydrationDailyTotalsQuery(userId, date, date.AddDays(1)), CancellationToken.None);
    ...
    public async Task GetHydrationEntriesQueryHandler_WithInvalidUserId_ReturnsInvalidToken() {
        var handler = new GetHydrationEntriesQueryHandler(CreateHydrationSender(new InMemoryHydrationEntryRepository()),
    ...
    public async Task GetHydrationEntriesQueryHandler_NormalizesDateAndMapsEntries() {
        ...
        await repository.AddAsync(HydrationEntry.Create(
            ...
            new DateTime(2026, 5, 27, 0, 0, 0, DateTimeKind.Utc),
            250));
        var handler = new GetHydrationEntriesQueryHandler(CreateHydrationSender(repository), CreateCurrentUserAccessService(user));
    ...
    private static IUserHydrationProfileReadService CreateHydrationProfileService(User user) {
        ...
        if (error is not null) {
            service
                .GetHydrationProfileAsync(user.Id, Arg.Any<CancellationToken>())
                .Returns(Task.FromResult(Result.Failure<UserHydrationProfileModel>(error)));
        }
