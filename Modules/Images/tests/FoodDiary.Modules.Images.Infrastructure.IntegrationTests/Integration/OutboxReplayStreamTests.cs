...
public sealed class OutboxReplayStreamTests(PostgresDatabaseFixture databaseFixture) {
    [RequiresDockerFact]
    public async Task Adapter_PreservesProjectionTrackingFiltersAndBoundedList() {
        ...
        Assert.Equal("image_object_deletion", stream.Name);
        Assert.Equal(1, stream.Order);
        Assert.Null(stream.ReplayRejectionReason);
        var message = ImageObjectDeletionOutboxMessage.Create("users/test/replay.webp", now.AddMinutes(-3));
        ...
        Assert.Multiple(
            ...
            () => Assert.Equal(message.ObjectKey, listed.Summary),
        ...
        Assert.Same(context.ChangeTracker.Entries<ImageObjectDeletionOutboxMessage>().Single().Entity, found.Message);
        ...
        var active = ImageObjectDeletionOutboxMessage.Create("users/test/replay.webp", now.AddMinutes(-3));
        var processed = ImageObjectDeletionOutboxMessage.Create("users/test/replay.webp", now.AddMinutes(-3));
