...
public partial class FastingFeatureTests {
    ...
    public async Task ProcessDueNotificationsAsync_WhenNoActiveOccurrences_ReturnsZeroWithoutPushes() {
        ...
        ISender scheduler = RequestTestSender.Create(new SendFastingNotificationsCommandHandler(
            ...
            new RecordingNotificationClientRefreshService(notificationPusher),
            CreateUnitOfWork(),
            new ImmediatePostCommitActionQueue(),
            new FixedDateTimeProvider(),
            NullLogger<SendFastingNotificationsCommandHandler>.Instance));

        int created = await scheduler.Send(new SendFastingNotificationsCommand(), CancellationToken.None);
    ...
    public async Task ProcessDueNotificationsAsync_WhenPostCommitQueueHasActions_FlushesQueue() {
        ...
        ISender scheduler = RequestTestSender.Create(new SendFastingNotificationsCommandHandler(
            ...
            new RecordingNotificationClientRefreshService(new RecordingNotificationPusher()),
            CreateUnitOfWork(),
    ...
    public async Task ProcessDueNotificationsAsync_WhenPastFollowUpThreshold_CreatesTwoRemindersOnceAndDeduplicatesLaterRuns() {
        ...
        int firstCreated = await scheduler.Send(new SendFastingNotificationsCommand(), CancellationToken.None);
        int secondCreated = await scheduler.Send(new SendFastingNotificationsCommand(), CancellationToken.None);
