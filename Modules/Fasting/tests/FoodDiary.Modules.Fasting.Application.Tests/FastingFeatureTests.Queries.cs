...
using FoodDiary.Modules.Fasting.Application.Queries.ReadCurrentFasting;
using FoodDiary.Modules.Fasting.Application.Queries.ReadFastingInsights;
using FoodDiary.Modules.Fasting.Application.Queries.ReadFastingOverview;
...
public partial class FastingFeatureTests {
    ...
    private static ISender CreateFastingReadService(
        IFastingOccurrenceReadModelRepository occurrenceRepository,
        IFastingCheckInReadModelRepository checkInRepository) =>
        RequestTestSender.Create(
            new ReadCurrentFastingQueryHandler(occurrenceRepository, checkInRepository),
            new ReadFastingInsightsQueryHandler(occurrenceRepository, new FastingAnalyticsService(occurrenceRepository, checkInRepository), new FixedDateTimeProvider()),
            new ReadFastingOverviewQueryHandler(occurrenceRepository, checkInRepository, new FastingAnalyticsService(occurrenceRepository, checkInRepository), new FixedDateTimeProvider()));
}
