...
public class ExportFeatureTests {
    ...
    public async Task ExportDiary_WithPdfFormat_ReturnsPdfFileResult() {
        ...
        var handler = new ExportDiaryQueryHandler(CreateMealExportReadService(meals), CreateCurrentUserAccessService(), pdfGenerator);
    ...
    public async Task ExportDiary_WithUnsafeReportOrigin_DropsOriginBeforePdfGeneration() {
        var userId = UserId.New();
        IDiaryPdfGenerator pdfGenerator = CreatePdfGenerator(out Func<(string? Locale, int? TimeZoneOffsetMinutes, string? ReportOrigin)> getPdfCall);
        var handler = new ExportDiaryQueryHandler(CreateMealExportReadService([]), CreateCurrentUserAccessService(), pdfGenerator);
    ...
    public async Task ExportDiary_ReadsOnlyLimitPlusSentinelAndRejectsOverflow() {
        ...
        Result<FileExportResult> result = await handler.Handle(
            new ExportDiaryQuery(userId.Value, TestDate, TestDate.AddDays(1)), CancellationToken.None);
