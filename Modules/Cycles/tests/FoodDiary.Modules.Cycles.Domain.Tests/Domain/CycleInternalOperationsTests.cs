...
public sealed class CycleInternalOperationsTests {
    [Fact]
    public void MenstrualEpisode_InternalOperations_EnforceRangesAndStatus() {
        DateOnly start = new(2026, 4, 10);
        Assert.Throws<ArgumentException>(() => CreateEpisode(CycleProfileId.Empty, start, end: null, MenstrualEpisodeStatus.Inferred));
        Assert.Throws<ArgumentOutOfRangeException>(() => CreateEpisode(CycleProfileId.New(), start, end: null, (MenstrualEpisodeStatus)999));
        Assert.Throws<ArgumentOutOfRangeException>(() => CreateEpisode(CycleProfileId.New(), start, start.AddDays(-1), MenstrualEpisodeStatus.Inferred));

        MenstrualEpisode inferred = CreateEpisode(CycleProfileId.New(), start, end: null, MenstrualEpisodeStatus.Inferred);
        ...
        Assert.Throws<InvalidOperationException>(() => InvokeInstance(inferred, "UpdateConfirmedRange", start, start.AddDays(1)));

        MenstrualEpisode confirmed = CreateEpisode(CycleProfileId.New(), start, end: null, MenstrualEpisodeStatus.Confirmed);
    ...
    public void CyclePredictionRevisionFactory_RejectsInvalidCoreValues() {
        ...
        Assert.Throws<ArgumentOutOfRangeException>(() => CreateRevision(CycleProfileId.New(), generatedAt, from, from, 1, 1, 50, reasonCodes: [new string('x', 600), new string('y', 600)]));

        CyclePredictionRevision revision = CreateRevision(CycleProfileId.New(), generatedAt, from, from.AddDays(2), 2, 1, 75, ["stable", "enough-data"]);
        Assert.Multiple(
    ...
    private static MenstrualEpisode CreateEpisode(CycleProfileId profileId, DateOnly start, DateOnly? end, MenstrualEpisodeStatus status) =>
    ...
    private static CyclePredictionRevision CreateRevision(CycleProfileId profileId, DateTime generatedAt, DateOnly? from, DateOnly? to, int completedCycles, int calibrationSamples, double? coverage, IReadOnlyCollection<string>? reasonCodes = null) =>
        InvokeStatic<CyclePredictionRevision>(typeof(CyclePredictionRevision), "Create", profileId, generatedAt, from, to, "high", "enough", "stable", completedCycles, calibrationSamples, coverage, 1d, reasonCodes!, "v1");

    private static T InvokeStatic<T>(Type type, string methodName, params object?[] arguments) => Invoke<T>(type.GetMethod(methodName, BindingFlags.Static | BindingFlags.NonPublic)!, instance: null, arguments);
