...
public partial class CyclesFeatureTests {
    ...
    [Fact]
    public async Task GetCurrentCycleQueryHandler_WithInjectedClock_UsesThatDateForPrediction() {
        var user = User.Create("cycle-fixed-clock@example.com", "hash");
        ...
        CycleModel model = Assert.IsType<CycleModel>(ResultAssert.Success(result));
        CyclePredictionsModel predictions = Assert.IsType<CyclePredictionsModel>(model.Predictions);
        Assert.Multiple(
            () => Assert.Equal(new DateOnly(2026, 4, 21), predictions.NextPeriodStartFrom),
            () => Assert.Equal(new DateOnly(2026, 4, 25), predictions.NextPeriodStartTo));
        clock.Received().GetUtcNow();
    }
