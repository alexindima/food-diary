...
public partial class CyclesFeatureTests {
    ...
    [InlineData(2, 2)]
    [InlineData(3, 3)]
    public async Task GetCycleNutritionSummaryQueryHandler_WithExcludedEpisode_DoesNotRestoreNearbyBleedingStart(
        int bleedingOffset, int expectedCycles) {
        var user = User.Create("cycle-excluded-nutrition@example.com", "hash");
        DateOnly start = new(2026, 1, 1);
        var profile = CycleProfile.Create(user.Id, start);
        profile.GrantConsent(CycleConsentPurpose.NutritionInsights, new DateTime(2026, 1, 1, 0, 0, 0, DateTimeKind.Utc));
        foreach (int offset in (int[])[0, 28 + bleedingOffset, 56, 84]) {
            profile.UpsertBleedingEntry(start.AddDays(offset), BleedingType.Bleeding, CycleFlowLevel.Medium, painImpact: null, notes: null);
        }
        MenstrualEpisode excluded = profile.ConfirmPeriodStart(start.AddDays(28));
        profile.UpdateMenstrualEpisode(excluded.Id, excluded.StartDate, excluded.EndDate, excludedFromPredictions: true);
        GetCycleNutritionSummaryQueryHandler handler = CreateCycleNutritionSummaryHandler(
            ...
            CreateStatisticsReadService([
                CreateNutritionBucket(start, 2100, 18),
                CreateNutritionBucket(start.AddDays(1), 1800, 28),
                CreateNutritionBucket(start.AddDays(28 + bleedingOffset), 2100, 18),
                CreateNutritionBucket(start.AddDays(29 + bleedingOffset), 1800, 28),
                CreateNutritionBucket(start.AddDays(56), 2100, 18),
                CreateNutritionBucket(start.AddDays(57), 1800, 28),
            ]),
            CreateCurrentUserAccessService(user));

        Result<CycleNutritionSummaryModel?> result = await handler.Handle(
            new GetCycleNutritionSummaryQuery(user.Id.Value, start, start.AddDays(84)), CancellationToken.None);

        CycleNutritionSummaryModel summary = Assert.IsType<CycleNutritionSummaryModel>(ResultAssert.Success(result));
        Assert.Multiple(
            () => Assert.Equal(expectedCycles, summary.CompletedCyclesAnalyzed),
            () => Assert.Equal(expectedCycles, summary.ComparableCycles),
            () => Assert.Equal(expectedCycles >= 3, summary.HasEnoughNutritionData));
    }
