...
public partial class NotificationsFeatureTests {
    ...
    private static IUserNotificationProfileService CreateNotificationPreferencesService(User user) =>
        CreateNotificationProfileService(user);
    ...
    private sealed class SingleUserRepository(User user) : IUserRepository, ICurrentUserAccessService {
        ...
        public Task<Result<User>> GetAccessibleUserAsync(UserId userId, CancellationToken cancellationToken) {
            ...
            if (foundUser is null) {
                return Task.FromResult(Result.Failure<User>(AuthenticationErrors.InvalidToken));
            }

            if (foundUser.DeletedAt is not null) {
                return Task.FromResult(Result.Failure<User>(UserAuthenticationErrors.AccountDeleted));
            }
