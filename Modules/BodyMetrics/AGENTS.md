...
- Keep `WeightEntry`, `WaistEntry`, and their IDs in module-owned `Domain` with their legacy CLR namespaces. The module depends one-way on central `User`/`UserId`; do not restore the removed inverse measurement navigations.
- Keep `WeightGoal`, `WaistGoal`, their IDs and lifecycle/status behavior, and the public goal navigations in central Domain as User-owned responsibilities.
