...
Domain references central Domain one-way for User/Recipe and their IDs. Neither has an inverse CLR navigation to these entities. Preserve existing navigation properties, WithMany mappings, cascade behavior and the unique user/recipe like index. Do not extract Recipes or introduce provider behavior changes.
