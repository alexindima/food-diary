...
public partial class MealsFeatureTests {
    ...
    private static UpdateMealCommandHandler CreateUpdateHandler(
        ...
        UpdateMealHandler(
            ...
            imageAccess ?? FoodDiary.Modules.Meals.Application.Tests.Support.AllowImageAssetAccessService.Instance);
    ...
    private sealed class FailingNonNullImageAssetAccessService : IImageAssetAccessService {
        public Task<Result<ImageAssetReadModel?>> ResolveOptionalAsync(
            ...
            Task.FromResult(
                assetId.HasValue
                    ? Result.Failure<ImageAssetReadModel?>(ImageErrors.Forbidden())
    ...
    private static ISender CreateFavoriteSender(IFavoriteMealRepository? favoriteMealRepository = null) {
        IFavoriteMealRepository repository = favoriteMealRepository ?? new StubFavoriteMealRepository();
        return RequestTestSender.Create(new ReadFavoriteMealsQueryHandler(repository), new ReadMealFavoriteStatusQueryHandler(repository), new ReadMealFavoriteIdsQueryHandler(repository), new ReadMealFavoritesOverviewQueryHandler(repository));
    }
    ...
    private sealed class StubFavoriteMealRepository(FavoriteMeal[]? favorites = null, Meal? meal = null) : IFavoriteMealRepository {
        ...
        public Task<(IReadOnlyList<FavoriteMealReadModel> Items, int TotalItems)> GetOverviewReadModelsAsync(
            ...
            Task.FromResult<(IReadOnlyList<FavoriteMealReadModel>, int)>((
                [.. _favorites.Take(limit).Select(ToReadModel)], _favorites.Length));
