using FoodDiary.Application.Abstractions.Meals.Models;
using FoodDiary.Application.Abstractions.Common.Models;
using FoodDiary.Application.Meals.Common;
using FoodDiary.Application.Meals.Commands.CreateMeal;
using FoodDiary.Application.Meals.Commands.DeleteMeal;
using FoodDiary.Application.Meals.Commands.RepeatMeal;
using FoodDiary.Application.Meals.Commands.UpdateMeal;
using FoodDiary.Application.Meals.Models;
using FoodDiary.Application.Meals.Queries.GetMealById;
using FoodDiary.Application.Meals.Queries.GetMeals;
using FoodDiary.Application.Meals.Queries.GetMealsOverview;
using FoodDiary.Presentation.Api.Features.Meals.Mappings;
using FoodDiary.Presentation.Api.Features.Meals.Requests;
using FoodDiary.Presentation.Api.Features.Meals.Responses;

namespace FoodDiary.Presentation.Api.Tests;

[ExcludeFromCodeCoverage]
public sealed class MealHttpMappingsTests {
    // â”€â”€ Command mappings â”€â”€

    [Fact]
    public void ToDeleteCommand_MapsIds() {
        var userId = Guid.NewGuid();
        var mealId = Guid.NewGuid();

        DeleteMealCommand command = mealId.ToDeleteCommand(userId);

        Assert.Equal(userId, command.UserId);
        Assert.Equal(mealId, command.MealId);
    }

    [Fact]
    public void CreateRequest_ToCommand_MapsAllFields() {
        var userId = Guid.NewGuid();
        DateTime date = DateTime.UtcNow;
        var productId = Guid.NewGuid();
        var request = new CreateMealHttpRequest(
            date, "Breakfast", "Tasty", ImageUrl: null, ImageAssetId: null,
            new List<MealItemHttpRequest> {
                new(productId, null, 150),
            },
            PreMealSatietyLevel: 3,
            PostMealSatietyLevel: 4);

        CreateMealCommand command = request.ToCommand(userId);

        Assert.Multiple(
            () => Assert.Equal(userId, command.UserId),
            () => Assert.Equal(date, command.Date),
            () => Assert.Equal("Breakfast", command.MealType),
            () => Assert.Equal("Tasty", command.Comment));
        Assert.Single(command.Items);
        Assert.Multiple(
            () => Assert.Equal(productId, command.Items[0].ProductId),
            () => Assert.Equal(150, command.Items[0].Amount),
            () => Assert.Equal(3, command.PreMealSatietyLevel),
            () => Assert.Equal(4, command.PostMealSatietyLevel));
    }

    [Fact]
    public void CreateRequest_ToCommand_WithAiSessions_MapsNestedItems() {
        var userId = Guid.NewGuid();
        var assetId = Guid.NewGuid();
        DateTime recognizedAt = DateTime.UtcNow;
        var request = new CreateMealHttpRequest(
            DateTime.UtcNow, MealType: null, Comment: null, ImageUrl: null, ImageAssetId: null,
            [],
            new List<MealAiSessionHttpRequest> {
                new(assetId, "Photo", recognizedAt, "AI notes", new List<MealAiItemHttpRequest> {
                    new("Chicken breast", "ÐšÑƒÑ€Ð¸Ð½Ð°Ñ Ð³Ñ€ÑƒÐ´ÐºÐ°", 200, "g", 330, 62, 7.2, 0, 0, 0),
                }),
            });

        CreateMealCommand command = request.ToCommand(userId);

        Assert.Single(command.AiSessions);
        Assert.Multiple(
            () => Assert.Equal(assetId, command.AiSessions[0].ImageAssetId),
            () => Assert.Equal(recognizedAt, command.AiSessions[0].RecognizedAtUtc),
            () => Assert.Equal("AI notes", command.AiSessions[0].Notes));
        Assert.Single(command.AiSessions[0].Items);
        Assert.Multiple(
            () => Assert.Equal("Chicken breast", command.AiSessions[0].Items[0].NameEn),
            () => Assert.Equal("ÐšÑƒÑ€Ð¸Ð½Ð°Ñ Ð³Ñ€ÑƒÐ´ÐºÐ°", command.AiSessions[0].Items[0].NameLocal),
            () => Assert.Equal(200, command.AiSessions[0].Items[0].Amount),
            () => Assert.Equal(330, command.AiSessions[0].Items[0].Calories));
    }

    [Fact]
    public void CreateRequest_ToCommand_WithNullAiSessions_MapsToEmptyList() {
        var request = new CreateMealHttpRequest(DateTime.UtcNow, MealType: null, Comment: null, ImageUrl: null, ImageAssetId: null, []);

        CreateMealCommand command = request.ToCommand(Guid.NewGuid());

        Assert.Empty(command.AiSessions);
    }

    [Fact]
    public void CreateRequest_ToCommand_WithNullLists_MapsEmptyCollections() {
        var request = new CreateMealHttpRequest(
            DateTime.UtcNow,
            MealType: "Breakfast",
            Comment: null,
            ImageUrl: null,
            ImageAssetId: null,
            Items: null!,
            AiSessions: null);

        CreateMealCommand command = request.ToCommand(Guid.NewGuid());

        Assert.Empty(command.Items);
        Assert.Empty(command.AiSessions);
    }

    [Fact]
    public void CreateRequest_ToCommand_PreservesNullNestedElementsForApplicationValidation() {
        var request = new CreateMealHttpRequest(
            DateTime.UtcNow,
            MealType: "Breakfast",
            Comment: null,
            ImageUrl: null,
            ImageAssetId: null,
            Items: [null!],
            AiSessions: [
                null!,
                new MealAiSessionHttpRequest(
                    ImageAssetId: null,
                    Source: "Text",
                    RecognizedAtUtc: DateTime.UtcNow,
                    Notes: null,
                    Items: [null!]),
            ]);

        CreateMealCommand command = request.ToCommand(Guid.NewGuid());

        Assert.Null(command.Items[0]);
        Assert.Null(command.AiSessions[0]);
        Assert.Null(command.AiSessions[1].Items[0]);
    }

    [Fact]
    public void UpdateRequest_ToCommand_MapsAllFields() {
        var userId = Guid.NewGuid();
        var mealId = Guid.NewGuid();
        var request = new UpdateMealHttpRequest(
            DateTime.UtcNow,
            "Lunch",
            Comment: null,
            ImageUrl: null,
            ImageAssetId: null,
            new List<MealItemHttpRequest> { new(null, Guid.NewGuid(), 2) },
            IsNutritionAutoCalculated: false,
            ManualCalories: 500,
            ManualProteins: 30,
            ManualFats: 20,
            ManualCarbs: 60,
            ManualFiber: 5,
            ManualAlcohol: 0);

        UpdateMealCommand command = request.ToCommand(userId, mealId);

        Assert.Multiple(
            () => Assert.Equal(userId, command.UserId),
            () => Assert.Equal(mealId, command.MealId),
            () => Assert.False(command.IsNutritionAutoCalculated),
            () => Assert.Equal(500, command.ManualCalories));
    }

    [Fact]
    public void UpdateRequest_ToCommand_WithNullAiItems_MapsEmptyCollection() {
        var request = new UpdateMealHttpRequest(
            DateTime.UtcNow,
            "Dinner",
            Comment: "ok",
            ImageUrl: null,
            ImageAssetId: null,
            Items: [
                new MealItemHttpRequest(Guid.NewGuid(), RecipeId: null, 150),
            ],
            AiSessions: [
                new MealAiSessionHttpRequest(
                    ImageAssetId: null,
                    Source: "Text",
                    RecognizedAtUtc: DateTime.UtcNow,
                    Notes: null,
                    Items: null!),
            ]);

        UpdateMealCommand command = request.ToCommand(Guid.NewGuid(), Guid.NewGuid());

        MealAiSessionInput aiSession = Assert.Single(command.AiSessions);
        Assert.Empty(aiSession.Items);
    }

    [Fact]
    public void RepeatMealRequest_ToRepeatCommand_MapsAllFields() {
        var userId = Guid.NewGuid();
        var mealId = Guid.NewGuid();
        DateTime targetDate = DateTime.UtcNow.AddDays(1);
        var request = new RepeatMealHttpRequest(targetDate, "Dinner");

        RepeatMealCommand command = request.ToRepeatCommand(userId, mealId);

        Assert.Multiple(
            () => Assert.Equal(userId, command.UserId),
            () => Assert.Equal(mealId, command.MealId),
            () => Assert.Equal(targetDate, command.TargetDate),
            () => Assert.Equal("Dinner", command.MealType));
    }

    // â”€â”€ Query mappings â”€â”€

    [Fact]
    public void GetMealsHttpQuery_ToQuery_MapsAllFields() {
        var userId = Guid.NewGuid();
        DateTime from = DateTime.UtcNow.AddDays(-7);
        DateTime to = DateTime.UtcNow;
        var httpQuery = new GetMealsHttpQuery(2, 20, from, to);

        GetMealsQuery query = httpQuery.ToQuery(userId);

        Assert.Multiple(
            () => Assert.Equal(userId, query.UserId),
            () => Assert.Equal(2, query.Page),
            () => Assert.Equal(20, query.Limit),
            () => Assert.Equal(from, query.DateFrom),
            () => Assert.Equal(to, query.DateTo));
    }

    [Fact]
    public void GetMealsHttpQuery_ToQuery_NormalizesFilters() {
        var userId = Guid.NewGuid();
        var httpQuery = new GetMealsHttpQuery(
            Page: 1,
            Limit: 10,
            DateFrom: null,
            DateTo: null,
            MealTypes: "Breakfast, dinner, breakfast, ,",
            CaloriesFrom: -1,
            CaloriesTo: 500,
            HasImage: true,
            HasAiSession: false);

        GetMealsQuery query = httpQuery.ToQuery(userId);

        Assert.Multiple(
            () => Assert.Equal(["Breakfast", "dinner"], query.MealTypes),
            () => Assert.Null(query.CaloriesFrom),
            () => Assert.Equal(500, query.CaloriesTo),
            () => Assert.True(query.HasImage),
            () => Assert.False(query.HasAiSession));
    }

    [Fact]
    public void GetMealsOverviewHttpQuery_ToQuery_NormalizesPagingAndFavoriteLimit() {
        var userId = Guid.NewGuid();
        DateTime from = DateTime.UtcNow.AddDays(-30);
        DateTime to = DateTime.UtcNow;
        var httpQuery = new GetMealsOverviewHttpQuery(
            Page: 0,
            Limit: 500,
            DateFrom: from,
            DateTo: to,
            FavoriteLimit: 0);

        GetMealsOverviewQuery query = httpQuery.ToQuery(userId);

        Assert.Multiple(
            () => Assert.Equal(userId, query.UserId),
            () => Assert.Equal(1, query.Page),
            () => Assert.Equal(100, query.Limit),
            () => Assert.Equal(from, query.DateFrom),
            () => Assert.Equal(to, query.DateTo),
            () => Assert.Equal(1, query.FavoriteLimit));
    }

    [Fact]
    public void MealId_ToQuery_MapsIds() {
        var userId = Guid.NewGuid();
        var mealId = Guid.NewGuid();

        GetMealByIdQuery query = mealId.ToQuery(userId);

        Assert.Equal(userId, query.UserId);
        Assert.Equal(mealId, query.MealId);
    }

    // â”€â”€ Response mappings â”€â”€

    [Fact]
    public void MealModel_ToHttpResponse_MapsTopLevelFields() {
        var id = Guid.NewGuid();
        DateTime date = DateTime.UtcNow;
        var model = new MealModel(
            id, date, "Breakfast", "Comment", ImageUrl: null, ImageAssetId: null,
            500, 30, 20, 60, 5, 0, IsNutritionAutoCalculated: true, ManualCalories: null, ManualProteins: null, ManualFats: null, ManualCarbs: null, ManualFiber: null, ManualAlcohol: null,
            3, 4, 72, "green", IsFavorite: false, FavoriteMealId: null, [], []);

        MealHttpResponse response = model.ToHttpResponse();

        Assert.Multiple(
            () => Assert.Equal(id, response.Id),
            () => Assert.Equal(date, response.Date),
            () => Assert.Equal("Breakfast", response.MealType),
            () => Assert.Equal(500, response.TotalCalories),
            () => Assert.Equal(3, response.PreMealSatietyLevel),
            () => Assert.Equal(4, response.PostMealSatietyLevel),
            () => Assert.Equal(72, response.QualityScore),
            () => Assert.Equal("green", response.QualityGrade),
            () => Assert.True(response.IsNutritionAutoCalculated));
        Assert.Empty(response.Items);
        Assert.Empty(response.AiSessions);
    }

    [Fact]
    public void MealModel_ToHttpResponse_MapsNestedItemsAndAiSessions() {
        var itemId = Guid.NewGuid();
        var mealId = Guid.NewGuid();
        var productId = Guid.NewGuid();
        var sessionId = Guid.NewGuid();
        var aiItemId = Guid.NewGuid();

        var items = new List<MealItemModel> {
            new(
                itemId,
                mealId,
                150,
                productId,
                "Chicken",
                "https://example.com/chicken.jpg",
                "g",
                100,
                165,
                31,
                3.6,
                0,
                0,
                0,
                null,
                null,
                null,
                null,
                null,
                null,
                null,
                null,
                null,
                null,
                85,
                "A",
                SourceAiItemId: aiItemId,
                Origin: "AiPhoto"),
        };
        var aiItems = new List<MealAiItemModel> {
            new(aiItemId, sessionId, "Rice", "Ð Ð¸Ñ", 200, "g", 260, 5, 0.6, 56, 0.4, 0),
        };
        var sessions = new List<MealAiSessionModel> {
            new(sessionId, mealId, null, null, "Text", DateTime.UtcNow, "Notes", aiItems),
        };

        var model = new MealModel(
            mealId, DateTime.UtcNow, MealType: null, Comment: null, ImageUrl: null, ImageAssetId: null,
            425, 36, 4.2, 56, 0.4, 0, IsNutritionAutoCalculated: true, ManualCalories: null, ManualProteins: null, ManualFats: null, ManualCarbs: null, ManualFiber: null, ManualAlcohol: null,
            0, 0, 61, "yellow", IsFavorite: false, FavoriteMealId: null, items, sessions);

        MealHttpResponse response = model.ToHttpResponse();

        Assert.Single(response.Items);
        Assert.Multiple(
            () => Assert.Equal(itemId, response.Items[0].Id),
            () => Assert.Equal(productId, response.Items[0].ProductId),
            () => Assert.Equal("Chicken", response.Items[0].ProductName),
            () => Assert.Equal("https://example.com/chicken.jpg", response.Items[0].ProductImageUrl),
            () => Assert.Equal(85, response.Items[0].ProductQualityScore),
            () => Assert.Equal("A", response.Items[0].ProductQualityGrade));

        Assert.Single(response.AiSessions);
        Assert.Equal(sessionId, response.AiSessions[0].Id);
        Assert.Single(response.AiSessions[0].Items);
        Assert.Multiple(
            () => Assert.Equal("Rice", response.AiSessions[0].Items[0].NameEn),
            () => Assert.Equal("Ð Ð¸Ñ", response.AiSessions[0].Items[0].NameLocal),
            () => Assert.Equal(260, response.AiSessions[0].Items[0].Calories));
    }

    [Fact]
    public void MealModel_ToHttpResponse_WithManualNutrition() {
        var model = new MealModel(
            Guid.NewGuid(), DateTime.UtcNow, MealType: null, Comment: null, ImageUrl: null, ImageAssetId: null,
            0, 0, 0, 0, 0, 0, IsNutritionAutoCalculated: false, 500, 30, 20, 60, 5, 0,
            0, 0, 55, "yellow", IsFavorite: false, FavoriteMealId: null, [], []);

        MealHttpResponse response = model.ToHttpResponse();

        Assert.Multiple(
            () => Assert.False(response.IsNutritionAutoCalculated),
            () => Assert.Equal(500, response.ManualCalories),
            () => Assert.Equal(30, response.ManualProteins));
    }

    [Fact]
    public void MealModel_ToHttpResponse_MapsFavoriteFields() {
        var favoriteMealId = Guid.NewGuid();
        var model = new MealModel(
            Guid.NewGuid(),
            DateTime.UtcNow,
            "Dinner",
            "Comment",
            ImageUrl: null,
            ImageAssetId: null,
            500,
            30,
            20,
            60,
            5,
            0,
            IsNutritionAutoCalculated: true,
            ManualCalories: null,
            ManualProteins: null,
            ManualFats: null,
            ManualCarbs: null,
            ManualFiber: null,
            ManualAlcohol: null,
            3,
            7,
            70,
            "green",
            IsFavorite: true,
            favoriteMealId,
            [],
            []);

        MealHttpResponse response = model.ToHttpResponse();

        Assert.True(response.IsFavorite);
        Assert.Equal(favoriteMealId, response.FavoriteMealId);
    }

    [Fact]
    public void MealOverviewModel_ToHttpResponse_MapsNestedCollections() {
        var meal = new MealModel(
            Guid.NewGuid(),
            DateTime.UtcNow,
            "Breakfast",
            Comment: null,
            ImageUrl: null,
            ImageAssetId: null,
            350,
            20,
            12,
            30,
            4,
            0,
            IsNutritionAutoCalculated: true,
            ManualCalories: null,
            ManualProteins: null,
            ManualFats: null,
            ManualCarbs: null,
            ManualFiber: null,
            ManualAlcohol: null,
            2,
            6,
            66,
            "yellow",
            IsFavorite: false,
            FavoriteMealId: null,
            [],
            []);
        var favorite = new MealFavoriteMealModel(
            Guid.NewGuid(),
            meal.Id,
            "Morning meal",
            DateTime.UtcNow,
            meal.Date,
            meal.MealType,
            meal.TotalCalories,
            meal.TotalProteins,
            meal.TotalFats,
            meal.TotalCarbs,
            1);
        var overview = new MealOverviewModel(
            new PagedResponse<MealModel>([meal], 1, 10, 1, 1),
            [favorite],
            1);

        MealOverviewHttpResponse response = overview.ToHttpResponse();

        Assert.Single(response.AllMeals.Data);
        Assert.Single(response.FavoriteItems);
        Assert.Multiple(
            () => Assert.Equal(1, response.FavoriteTotalCount),
            () => Assert.Equal(meal.Id, response.AllMeals.Data[0].Id),
            () => Assert.Equal(favorite.Id, response.FavoriteItems[0].Id));
    }
}
