...
public sealed class TelegramOperationHandlersTests {
    [Theory]
    [InlineData("missing")]
    [InlineData("conflict")]
    [InlineData("created")]
    ...
    public async Task Checkpoint_PropagatesAtomicStoreOutcome(bool stored) {
        ...
        Result result = await CreateSender().Send(new CheckpointTelegramOperationCommand(lease.OperationId, lease.LeaseId, "result", Completed: true, DateTime.UtcNow), CancellationToken.None);
    ...
    public TelegramOperationHandlersTests() {
    ...
    public async Task StaleGeneration_CancelsOnlyStaleOperationAndDoesNotExposePayload() {
        ...
        Result<TelegramOperationLease> result = await CreateSender().Send(new AcquireTelegramOperationCommand(lease.OperationId), CancellationToken.None);
    ...
    private ISender CreateSender() => RequestTestSender.Create(
        new RegisterTelegramOperationCommandHandler(_store, _policy, _identities, TimeProvider.System),
        new AcquireTelegramOperationCommandHandler(_store, _policy, _identities, TimeProvider.System),
        new CheckpointTelegramOperationCommandHandler(_store, _policy, _identities, TimeProvider.System));
}
