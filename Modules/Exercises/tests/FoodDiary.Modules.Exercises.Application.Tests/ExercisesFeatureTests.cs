...
public class ExercisesFeatureTests {
    ...
    public async Task GetExerciseEntries_ReturnsModels() {
        ...
        var handler = new GetExerciseEntriesQueryHandler(RequestTestSender.Create(new ReadExerciseEntriesQueryHandler(repo)), Substitute.For<ICurrentUserAccessService>());
    ...
    public async Task ExerciseEntryReadService_MapsReadModels() {
        ...
        var repo = new InMemoryExerciseEntryRepository();
        repo.Seed(entry);
        ISender service = RequestTestSender.Create(new ReadExerciseEntriesQueryHandler(repo), new ReadExerciseCaloriesQueryHandler(repo));

        IReadOnlyList<ExerciseEntryModel> result = await service.Send(new ReadExerciseEntriesQuery(userId, DateTime.UtcNow.AddDays(-1), DateTime.UtcNow.AddDays(1)), CancellationToken.None);
    ...
    public async Task GetExerciseEntries_WithNullUserId_ReturnsFailure() {
        var handler = new GetExerciseEntriesQueryHandler(RequestTestSender.Create(new ReadExerciseEntriesQueryHandler(new InMemoryExerciseEntryRepository())), Substitute.For<ICurrentUserAccessService>());
