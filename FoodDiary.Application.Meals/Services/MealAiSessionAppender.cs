...
internal static class MealAiSessionAppender {
    public static async Task<Result> AddAsync(
        Meal meal,
        IEnumerable<MealAiSessionInput> sessions,
        UserId userId,
        IImageAssetAccessService imageAssetAccessService,
        TimeProvider dateTimeProvider,
        CancellationToken cancellationToken) {
        foreach (MealAiSessionInput session in sessions) {
    ...
    private static Result<List<MealAiItemData>> CreateAiSessionItems(MealAiSessionInput session) {
        var items = new List<MealAiItemData>(session.Items.Count);
        foreach (MealAiItemInput aiItem in session.Items) {
