...
public static class HydrationHttpQueryMappings {
    public static GetHydrationEntriesQuery ToEntriesQuery(this GetHydrationEntriesHttpQuery query, Guid userId) {
        return new GetHydrationEntriesQuery(userId, query.DateUtc ?? DateTime.UtcNow);
    }

    public static GetHydrationDailyTotalQuery ToDailyQuery(this GetHydrationEntriesHttpQuery query, Guid userId) {
