...
- idempotent POST behavior is now also explicitly covered in `FoodDiary.Presentation.Api.Tests` and called out in API governance for critical write paths
...
- moved role-aware application usage behind `User.GetRoleNames()` and `User.HasRole(...)` so application code no longer traverses `UserRoles` directly in key paths
...
- current-user lifecycle policy now also covers core CRUD flows for products, recipes, consumptions, and shopping lists
- deleted-user cleanup now rejects deleted or inactive reassignment targets and falls back to the delete path
- admin and restore lifecycle transitions now have explicit guard/test coverage for deleted-user state
