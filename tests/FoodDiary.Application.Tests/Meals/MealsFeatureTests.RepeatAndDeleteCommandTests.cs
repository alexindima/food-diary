...
public partial class MealsFeatureTests {
    ...
    public async Task DeleteMealCommandHandler_WithEmptyMealId_ReturnsValidationFailure() {
        DeleteMealCommandHandler handler = DeleteMealHandler(new CreatingMealRepository());

        Result result = await handler.Handle(
            new DeleteMealCommand(Guid.NewGuid(), Guid.Empty),
    ...
    [Fact]
    public async Task DeleteMealCommandHandler_WhenMealIsMissing_ReturnsNotFound() {
        DeleteMealCommandHandler handler = DeleteMealHandler(new CreatingMealRepository());

        Result result = await handler.Handle(
            new DeleteMealCommand(Guid.NewGuid(), Guid.NewGuid()),
            CancellationToken.None);
    ...
    [Fact]
    public async Task DeleteMealCommandHandler_WhenMealExists_DeletesMeal() {
        var user = User.Create("delete-meal@example.com", "hash");
        ...
        DeleteMealCommandHandler handler = DeleteMealHandler(repository);

        Result result = await handler.Handle(
            new DeleteMealCommand(user.Id.Value, meal.Id.Value),
            CancellationToken.None);
    ...
    [Fact]
    public async Task RepeatMealCommandHandler_WithAiAndManualNutrition_CopiesFullMeal() {
    ...
    [Fact]
    public async Task DeleteMealCommandHandler_WithMissingUserId_ReturnsInvalidToken() {
        DeleteMealCommandHandler handler = DeleteMealHandler(new CreatingMealRepository());

        Result result = await handler.Handle(
            new DeleteMealCommand(UserId: null, Guid.NewGuid()),
            CancellationToken.None);
