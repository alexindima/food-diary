...
public sealed class ContainerSupplyChainGuardrailTests {
    ...
    public void DeployWorkflow_AuthenticatesSshHostAndRequiresProductionCredentials() {
        ...
        Assert.Multiple(
            ...
            () => Assert.DoesNotContain("MailRelayBroker__Password MailRelay__RequireMailgunWebhookSignature", workflow, StringComparison.Ordinal),
            () => Assert.Contains("Mailgun ingestion requires MailRelay__MailgunWebhookSigningKey", workflow, StringComparison.Ordinal),
            () => Assert.Contains("MailRelay__RequireMailgunWebhookSignature must be true or false", workflow, StringComparison.Ordinal),
            () => Assert.Contains("Production credential must not use dotenv interpolation or command substitution", workflow, StringComparison.Ordinal),
            ...
            () => Assert.DoesNotContain("Production requires MailRelay__RequireMailgunWebhookSignature=true", workflow, StringComparison.Ordinal),
