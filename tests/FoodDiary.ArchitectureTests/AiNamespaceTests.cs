...
public sealed class AiNamespaceTests {
    [Theory]
    ...
    [InlineData("tests/FoodDiary.Modules.Ai.Application.Tests")]
    [InlineData("tests/FoodDiary.Modules.Ai.Domain.Tests")]
    [InlineData("tests/FoodDiary.Modules.Ai.Infrastructure.Tests")]
    [InlineData("tests/FoodDiary.Modules.Ai.Presentation.Tests")]
    public void Projects_UseProjectNamesAndFolderNamespaces(string project) {
        string directory = ArchitectureTestPaths.FromRoot("Modules", "Ai", project);
        ...
        string expectedRoot = project.StartsWith("tests/", StringComparison.Ordinal)
            ? project[6..] : "FoodDiary.Modules.Ai." + project;
