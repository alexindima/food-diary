...
public sealed class PersistenceTransactionGuardrailTests {
    ...
    public void PersistenceSaveChangesAsyncUsage_StaysInsideCurrentExplicitAllowlist() {
        ...
        string[] allowedFiles = [
            Path.Combine(persistenceRoot, "Admin", "AdminImpersonationSessionRepository.cs"),
            Path.Combine(persistenceRoot, "Admin", "EmailTemplateRepository.cs"),
            Path.Combine(persistenceRoot, "Ai", "AiPromptTemplateRepository.cs"),
            ...
            Path.Combine(persistenceRoot, "Content", "NutritionLessonRepository.cs"),
            Path.Combine(persistenceRoot, "Dietologist", "DietologistInvitationRepository.cs"),
            Path.Combine(persistenceRoot, "EfUnitOfWork.cs"),
            Path.Combine(persistenceRoot, "Images", "ImageAssetRepository.cs"),
            Path.Combine(persistenceRoot, "Meals", "MealRepository.cs"),
            ...
            Path.Combine(persistenceRoot, "Products", "ProductRepository.cs"),
            Path.Combine(persistenceRoot, "RecentItems", "RecentItemRepository.cs"),
            Path.Combine(persistenceRoot, "Recipes", "RecipeRepository.cs"),
            Path.Combine(persistenceRoot, "Recommendations", "RecommendationRepository.cs"),
            Path.Combine(persistenceRoot, "Tracking", "CycleRepository.cs"),
            ...
            Path.Combine(persistenceRoot, "Wearables", "WearableConnectionRepository.cs"),
            Path.Combine(persistenceRoot, "Wearables", "WearableSyncRepository.cs"),
        ];
