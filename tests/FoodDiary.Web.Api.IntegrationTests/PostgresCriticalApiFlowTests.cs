...
public sealed class PostgresCriticalApiFlowTests(PostgresApiWebApplicationFactory factory)
    ...
    public async Task Register_ThenAccessProtectedEndpoint_WorksAgainstPostgres() {
        ...
        var usersInfoResponse = await client.GetAsync("/api/v1/users/info");
    ...
    public async Task DeleteProduct_KeepsShoppingListItemButClearsProductReferenceAgainstPostgres() {
        ...
        var deleteProductResponse = await client.DeleteAsync($"/api/v1/products/{product.Id}");
        var currentShoppingListResponse = await client.GetAsync("/api/v1/shopping-lists/current");
    ...
    public async Task DeleteImageAsset_BlocksReferencedRecipeAssets_ThenReturnsNotFoundAfterRecipeDeletionAgainstPostgres() {
        ...
        var deleteRecipeAssetWhileInUse = await client.DeleteAsync($"/api/v1/images/{recipeAsset.AssetId}");
        var deleteStepAssetWhileInUse = await client.DeleteAsync($"/api/v1/images/{stepAsset.AssetId}");
        ...
        var deleteRecipeResponse = await client.DeleteAsync($"/api/v1/recipes/{recipe.Id}");
        var deleteRecipeAssetAfterRecipeDeletion = await client.DeleteAsync($"/api/v1/images/{recipeAsset.AssetId}");
        var deleteStepAssetAfterRecipeDeletion = await client.DeleteAsync($"/api/v1/images/{stepAsset.AssetId}");
