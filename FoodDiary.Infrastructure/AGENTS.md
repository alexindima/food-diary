...
EfModuleTransactionCoordinator.ExecuteItemAsync owns the Users cleanup per-item transaction policy, including clean entry, whole-attempt retries, all-owner reset, and unconditional successful save. It preserves the former null post-commit queue. FoodDiaryDbContext explicitly implements IModuleChangeTrackerSource to inspect live entries only in requested owner context types. Module audit interceptors do not resolve the concrete shared context through DI. Users and Dietologist Infrastructure no longer reference this project or receive its InternalsVisibleTo grants. Dietologist retains only the existing narrow shared audit persistence model access.
...
No module Infrastructure depends on this assembly. JwtOptions belongs to shared Authentication.Contracts and is still bound here; quota-orphan telemetry belongs to Ai Infrastructure with its original meter identity. Migration composition consumes the canonical Identity PersistenceModel.
