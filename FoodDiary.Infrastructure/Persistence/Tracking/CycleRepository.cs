...
public sealed class CycleRepository(FoodDiaryDbContext context) : ICycleRepository {
    private static readonly Expression<Func<CycleProfile, CycleProfileReadModel>> ReadModelProjection = profile =>
        new CycleProfileReadModel(
            ...
            profile.Notes,
            profile.BleedingEntries.Select(entry => new BleedingEntryReadModel(
                entry.Id.Value, entry.CycleProfileId.Value, entry.Date, entry.Type, entry.Flow, entry.PainImpact, entry.Notes)).ToList(),
            profile.SymptomEntries.Select(entry => new CycleSymptomEntryReadModel(
                entry.Id.Value, entry.CycleProfileId.Value, entry.Date, entry.Category, entry.Intensity, entry.Tags, entry.Note)).ToList(),
            profile.Factors.Select(factor => new CycleFactorReadModel(
                factor.Id.Value, factor.CycleProfileId.Value, factor.Type, factor.StartDate, factor.EndDate, factor.Notes)).ToList(),
            profile.FertilitySignals.Select(signal => new FertilitySignalReadModel(
                signal.Id.Value, signal.CycleProfileId.Value, signal.Date, signal.BasalBodyTemperatureCelsius,
