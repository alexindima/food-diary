...
ExecuteItemAsync is the existing independently committed batch-item policy: one provider retry boundary, all-owner reset, unconditional UoW save on true and commit without save on false. It deliberately does not inspect/discard post-commit actions. Keep it distinct from command and result transaction overloads. IModuleChangeTrackerSource provides live typed owner entries for coordinated save interceptors; it exposes no context registry, connection, transaction or save method. Application/Domain must not consume it.
