...
public sealed class PersistenceRepositoryCoverageIntegrationTests(PostgresDatabaseFixture databaseFixture) {
    ...
    public async Task TrackingRepositories_CoverDateFiltersTotalsAndFastingQueries() {
        ...
        Assert.Null(await exerciseRepository.GetByIdAsync(exercise.Id, UserId.New(), asTracking: true));
        Assert.Empty(await exerciseRepository.GetByDateRangeReadModelsAsync(UserId.New(), today, today));
...
#pragma warning disable MA0004
    ...
    private static async Task CoverFastingOccurrenceAndCheckInsAsync(
        ...
        DateTime now) {
        var occurrenceRepository = new FastingOccurrenceRepository(context.FastingOccurrences, new FoodDiary.Modules.Users.Infrastructure.Persistence.Users.UserRelatedDataReadService(context.Users));
